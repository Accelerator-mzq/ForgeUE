# Delta Spec: provider-routing (cleanup-main-spec-scenarios)

> 给 `openspec/specs/provider-routing/spec.md` 的 16 个已有 Requirement 补 `#### Scenario:` 块(共 17 个 Scenario,其中 `Pricing probe defaults to dry-run` 写 2 个 Scenario 覆盖 dry-run 与 `--apply` 两侧)。**不**新增 Requirement,**不**改 Requirement 标题或描述(本 capability 无 [审视] 项)。`Wildcard adapter is registered last` 已有 Scenario,不在本 delta 范围。
