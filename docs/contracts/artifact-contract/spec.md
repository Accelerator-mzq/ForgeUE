...
1. `ComfyAgentWorker.generate_mesh(...)` returns `MeshCandidate(data=<glb header bytes>, source_path=<ComfyUI output GLB path>, format="glb", mime_type="model/gltf-binary", metadata={"comfy_manifest": <name>, "comfy_params_snapshot": <dict copy>, "comfy_capability": "mesh", "comfy_original_filename": <name>, "comfy_input_filename": <staging 绝对路径或 legacy 裸文件名>})` — populating the existing `MeshCandidate.metadata: dict[str, Any]` field and the FOR-13 `source_path` field at `mesh_worker.py`. (2026-06-11 `comfy-agent-api-v3-adaptation` 修订:`comfy_input_filename` 值为 executor in-tree staging 绝对路径 `<run_dir>/comfy/forgeue_<sha1>.png`,CLI 侧 auto-upload 到 ComfyUI input/;原 round 5 的 `comfy_input_dir` 字段随 `FORGEUE_COMFY_INPUT_DIR` 退役删除;真源 source image artifact id 仍可通过 `Artifact.lineage.source_artifact_ids` 追溯。) `data` is retained for fake / remote workers and contains only header bytes on the Comfy local path.
