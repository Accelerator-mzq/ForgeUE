...
- **Engine export (export step)**: `framework.runtime.executors.export.ExportExecutor` resolves `engine_target` / legacy `ue_target` and dispatches through `EngineAdapterRegistry`; each adapter owns its downstream validation. `UnrealAdapter` reuses `framework.engine_bridge.unreal.contract.manifest_builder.build_manifest` and `validate_manifest(...)`;`framework.ue_bridge` was removed after FOR-32 and is not a current contract entry. `Godot4Adapter` validates supported file-backed shapes plus fresh `.import` / `.godot/imported` outputs.
- **Unreal contract inspection**: `framework.engine_bridge.unreal.contract.inspect` — `inspect_project / inspect_content_path / inspect_asset_exists / validate_manifest` (`src/framework/engine_bridge/unreal/contract/inspect/project.py`) run pre-import checks at the Unreal adapter boundary.
...
1. `ComfyAgentWorker.generate_mesh(...)` returns `MeshCandidate(data=<glb header bytes>, source_path=<ComfyUI output GLB path>, format="glb", mime_type="model/gltf-binary", metadata={"comfy_manifest": <name>, "comfy_params_snapshot": <dict copy>, "comfy_capability": "mesh", "comfy_original_filename": <name>, "comfy_input_filename": <staging 绝对路径或 legacy 裸文件名>, "comfy_prompt_id": <detach submit 返回的 prompt id, 自 `comfy-detach-wait-adoption` 2026-06-11>})` — populating the existing `MeshCandidate.metadata: dict[str, Any]` field and the FOR-13 `source_path` field at `mesh_worker.py`. (2026-06-11 `comfy-agent-api-v3-adaptation` 修订:`comfy_input_filename` 值为 executor in-tree staging 绝对路径 `<run_dir>/comfy/forgeue_<sha1>.png`,CLI 侧 auto-upload 到 ComfyUI input/;原 round 5 的 `comfy_input_dir` 字段随 `FORGEUE_COMFY_INPUT_DIR` 退役删除;真源 source image artifact id 仍可通过 `Artifact.lineage.source_artifact_ids` 追溯。) `data` is retained for fake / remote workers and contains only header bytes on the Comfy local path.
...
**Then** the returned `MeshCandidate.metadata` contains `comfy_manifest="3D_Hunyuan/3d_hunyuan3d-v2.1"`(round 5 修订:用实际可用 manifest), `comfy_params_snapshot={"seed": 42, "input_image": "<staging 绝对路径>"}` (2026-06-11 修订:值为 in-tree staging 绝对路径,上游 CLI auto-upload 后在 server 侧重写为相对名;snapshot taken AFTER worker injects;mutating the original `spec["comfy_params"]` after the call does NOT change `metadata["comfy_params_snapshot"]`), `comfy_capability="mesh"`, `comfy_original_filename=<ComfyUI 输出 GLB 文件名>`, `comfy_input_filename="<staging 绝对路径>"`, `comfy_prompt_id=<detach submit 返回的 prompt id>`; the `MeshCandidate` dataclass type is `mesh_worker.MeshCandidate` unchanged

## Scenario: GenerateMeshExecutor persists ComfyAgentWorker mesh candidates via repo.put with worker_metadata

**Given** `_generate_via_comfy_worker` returns `[MeshCandidate(data=<glb_header>, source_path="<ComfyUI output>/asset_00001_.glb", format="glb", mime_type="model/gltf-binary", metadata={"comfy_manifest": "3D_Hunyuan/3d_hunyuan3d-v2.1", "comfy_params_snapshot": {...}, "comfy_capability": "mesh", "comfy_original_filename": "asset_00001_.glb", "comfy_input_filename": "<staging 绝对路径>", "comfy_prompt_id": <prompt id>})]` from a step whose `ctx.repository` is a real `ArtifactRepository` rooted at `<artifact_root>/<run_id>/`
...
## Requirement: Mesh worker source image bytes are staged in-tree before subprocess invocation (2026-06-11 `comfy-agent-api-v3-adaptation` 修订)

The system SHALL guarantee that when `GenerateMeshExecutor` dispatches to the comfy-worker branch (`_should_use_comfy_worker_path(ctx)` returns True), the upstream source image bytes resolved by `_resolve_source_image(ctx)` are written to the **in-tree staging file** `(ctx.run_dir / "comfy").resolve() / "forgeue_<sha1_hex>.png"` (where `<sha1_hex>` is `hashlib.sha1(source_bytes).hexdigest()[:16]`, providing idempotency) before the worker subprocess is invoked. The **resolved absolute path** SHALL be passed to `ComfyAgentWorker.generate_mesh(source_image_filename=...)`, which injects it into `spec["comfy_params"][<image_param_key>]`(default `"input_image"`). Upstream `comfyui_api run`(v3)auto-uploads `input_image*` local-path params via POST /upload/image — ForgeUE no longer writes into the ComfyUI installation's `input/` directory(原 round 5 D10 `FORGEUE_COMFY_INPUT_DIR` 机制退役;残留 env/yaml 键被容忍并忽略;原"输入副本 cleanup 由用户管"义务随之转移到上游 input 目录管理)。

**NFR-PORT-004 适用范围(2026-06-11 更新)**:
...
- staging 文件本身现在**就在**项目树内(`<run_dir>/comfy/forgeue_<sha1>.png`,随 run 产物生命周期管理);上游 upload 到 ComfyUI input/ 的副本属 ComfyUI 域 ephemeral 文件,不影响 `tar artifacts/<run_id>/` self-contained 性

## Scenario: Source image bytes staged in-tree with sha1-derived filename; absolute path passed to worker

**Given** an upstream `_resolve_source_image(ctx)` call returns `(source_bytes=b"<png bytes>", source_image_artifact_id="run_X_step_image_1")` where `hashlib.sha1(b"<png bytes>").hexdigest()[:16] == "abc123def456"`; `ctx.run_dir` may be relative(framework.run 默认 `artifacts/<today>/<run_id>`)
**When** `_generate_via_comfy_worker(ctx, spec, source_image_bytes=source_bytes, source_image_artifact_id=..., ...)` is invoked
**Then** the executor writes the bytes to `<resolved run_dir>/comfy/forgeue_abc123def456.png` (creating the directory if missing); the file content equals `source_bytes` exactly; subsequent calls in the same run with identical bytes do NOT re-write (idempotent via hash); `worker.generate_mesh` is invoked with `source_image_filename="<该文件绝对路径>"`(`resolve()` 强制 — CLI 子进程 cwd=scripts_dir,相对路径必致 auto-upload 不触发,fence `test_generate_via_comfy_worker_resolves_relative_run_dir_to_absolute_path`); the `comfy_params` passed to the subprocess contains `input_image: "<绝对路径>"`,上游 CLI auto-upload 后在 server 侧重写为相对名

## Scenario: Mesh path succeeds without FORGEUE_COMFY_INPUT_DIR (退役 fence)

**Given** env var `FORGEUE_COMFY_INPUT_DIR` unset AND `provider_config` without `input_dir`
...
1. **Provenance contract** (mirrors Phase 1 mesh `Artifact.metadata["worker_metadata"]` modeling):
   - `Artifact.metadata["worker_metadata"]` SHALL contain `{"comfy_manifest": <manifest name>, "comfy_params_snapshot": <dict copy of spec.comfy_params at call time>, "comfy_capability": "audio", "comfy_original_filename": <ComfyUI-side filename, e.g. "ComfyUI_00001_.flac">, "comfy_prompt_id": <detach submit 返回的 prompt id, 自 `comfy-detach-wait-adoption` 2026-06-11>, "comfy_subprocess_run_metadata": {<exit_code, total_seconds, ...>}}` — populated by `ComfyAgentWorker.generate_audio` into `AudioCandidate.metadata` and copied through `repo.put(metadata={"worker_metadata": dict(cand.metadata), ...})` by `GenerateAudioExecutor`
...
**Then** the returned `AudioCandidate.metadata` contains `{"comfy_manifest": "Audio_Workflows/audio_stable_audio_example", "comfy_params_snapshot": {"text": "uplifting electronic music", "duration_seconds": 10.0, "seed": 42, "steps": 50}, "comfy_capability": "audio", "comfy_original_filename": "ComfyUI_00001_.flac", "comfy_prompt_id": <detach submit 返回的 prompt id>, "comfy_subprocess_run_metadata": {...}}`; mutating the original `spec["comfy_params"]` dict after the call does NOT change `metadata["comfy_params_snapshot"]` (snapshot is a `dict(...)` copy); `tests/unit/test_comfy_subprocess.py::test_generate_audio_metadata_snapshot_is_independent_copy` fences this
...
1. **Provenance contract** (mirrors audio Phase 2 `Artifact.metadata["worker_metadata"]` modeling):
   - `Artifact.metadata["worker_metadata"]` SHALL contain `{"comfy_manifest": <manifest name>, "comfy_params_snapshot": <dict copy of spec.comfy_params at call time>, "comfy_capability": "video", "comfy_original_filename": <ComfyUI-side filename, e.g. "wan21_1.3b_5sec_00001.mp4">, "comfy_prompt_id": <detach submit 返回的 prompt id, 自 `comfy-detach-wait-adoption` 2026-06-11>, "comfy_subprocess_run_metadata": {<exit_code, total_seconds, ...>}}` — populated by `ComfyAgentWorker.generate_video` into `VideoCandidate.metadata` and copied through `repo.put(metadata={"worker_metadata": dict(cand.metadata), ...})` by `GenerateVideoExecutor`
