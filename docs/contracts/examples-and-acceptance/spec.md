...
**Given** a host with ComfyUI + Stable Audio Open model weights cached + `FORGEUE_COMFY_SCRIPTS_DIR` configured + `python -m comfyui_api serve` running(v3.3 起 comfyui_api 自带 serve;`factory_v3 serve` 仍是兼容入口)
...
- Top-level `steps` array containing exactly one Step object:
  ...
  - `config`: executor-specific free-form dict containing:
    ...
    - `spec.comfy_workflow`: **`"Vedio/Wan2.1-T2V-1.3B_native_teacache"`** (2026-06-11 `comfy-agent-api-v3-adaptation` 起 smoke 默认切 TeaCache 加速变体,L2 单次 ~2 分钟;D5: `Vedio/` upstream拼写照实跟随,**不**做翻译;`Vedio/Wan2.1-T2V-1.3B_native_5sec` 留 `cluster2_l2_video_export.json` 与 probe baseline,users MAY swap to `Vedio/Wan2.2-T2V-A14B_GGUF` for higher quality with 14+ GB VRAM)
...
**Given** a host with ComfyUI + Wan 2.1 1.3B model weights cached + `FORGEUE_COMFY_SCRIPTS_DIR` configured + `python -m comfyui_api serve` running(v3.3 起 comfyui_api 自带 serve;`factory_v3 serve` 仍是兼容入口) (ComfyUI pre-warmed)
...
- Run `<UE_path>/Engine/Binaries/Win64/UnrealEditor-Cmd.exe <project>.uproject -ExecutePythonScript="<repo>/engine_scripts/unreal/run_import.py"` with `FORGEUE_RUN_FOLDER` env pointing to the artifact run folder (Bash 直接驱动,Claude 不需要用户手工点 Python Console)
...
**When** the operator invokes `UnrealEditor-Cmd.exe <project>.uproject -ExecutePythonScript="<repo>/engine_scripts/unreal/run_import.py"` from a Bash shell
