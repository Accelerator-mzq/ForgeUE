...
- THEN the executor takes the comfy-worker dispatch branch and calls `_generate_via_comfy_worker(...)` which constructs `ComfyAgentWorker(model_id="comfy/local-audio", ...)` inline (NO source bytes write to ComfyUI input/, NO `_resolve_source_image` call, NO `FORGEUE_COMFY_INPUT_DIR` read) and invokes `worker.generate_audio(spec=spec, num_candidates=num, seed=seed, timeout_s=timeout_s)`; `_capability="audio"` is inferred; output validation requires `outputs.audio` non-empty and rejects `outputs.images / glb / video`; returned `AudioCandidate`s carry comfy provenance in `metadata={comfy_manifest, comfy_params_snapshot, comfy_capability="audio", comfy_original_filename, comfy_prompt_id, comfy_subprocess_run_metadata}` plus `source_path`, and are persisted via `repo.put(source_path=cand.source_path, payload_kind=PayloadKind.file, file_suffix=f".{cand.format}", metadata={"worker_metadata": dict(cand.metadata), ...})` when present, falling back to byte-value persistence for fake / remote candidates
...
The system SHALL invoke ComfyUI through a **two-stage detach+wait subprocess protocol**(自 `comfy-detach-wait-adoption` 2026-06-11):第一段 submit — `python -m comfyui_api run --workflow X --params P --project ID --lifecycle none --timeout T --detach` 立即返回含 `prompt_id` 的 JSON(wall-clock 上限 `_SUBMIT_TIMEOUT_S=60s`;响应缺 `prompt_id` → `WorkerUnsupportedResponse`);第二段 wait — `python -m comfyui_api wait --prompt-id <id> --timeout T` 收割 `outputs`(wall-clock = T+30s)。两段共走低层单子进程封装 `_invoke_comfy_cli_once`(spawn / communicate / cleanup / decode / JSON / ok 分类),orchestration 由 `_run_comfy_prompt`(锁内 submit→wait 全程串行)负责。此协议替代原单阻塞 `comfyui_api run` 子进程方式,不再用 HTTP 直调 `/prompt` + `/history` + `/view`。The worker class `ComfyAgentWorker` SHALL accept the following constructor parameters (keyword-only):
...
Each call SHALL use the detach+wait two-stage protocol(自 `comfy-detach-wait-adoption` 2026-06-11):submit `--workflow <manifest_name>` + `--params <json>` + `--project <task.project_id>` + `--lifecycle none` + `--timeout <s>` + `--detach`(立即返回含 `prompt_id` 的 JSON);wait `--prompt-id <id>` + `--timeout <s>`(收割 `outputs`,其 `outputs.<key>` field carries absolute paths per the resolved capability:`outputs.images` for image-mode, `outputs.glb` for mesh-mode, `outputs.audio` for audio-mode, `outputs.video` for video-mode). The worker MUST NOT speak ComfyUI HTTP directly. The dispatch-method-by-capability table is:
...
All three methods share:①低层单子进程封装 `_invoke_comfy_cli_once`(spawn / communicate / cleanup / decode / JSON / ok 分类;自 `comfy-detach-wait-adoption` 2026-06-11 引入,替代原 `_run_subprocess_and_validate`)+ ② orchestration `_run_comfy_prompt`(锁内 submit→wait 全程串行)+ ③ capability-aware `_validate_outputs(outputs)`。4 条 `_run_once_*_async` 方法(image / mesh / audio / video)全部经由 `_invoke_comfy_cli_once` → `_run_comfy_prompt` 路径,防止 drift;`test_all_four_run_once_methods_route_through_shared_cli_helper` fence 守门。
...
1. Invoke the shared detach+wait orchestration via `_run_comfy_prompt(spec, timeout_s) -> dict`(自 `comfy-detach-wait-adoption` 2026-06-11;底层走 `_invoke_comfy_cli_once` 两次调用:submit `--detach` + wait `--prompt-id`;替代原单阻塞 `_run_subprocess_and_validate`)。submit `python -m comfyui_api run --workflow <manifest> --params <json> --project <task.project_id> --lifecycle none --timeout <s> --detach`(返回含 `prompt_id` 的 JSON);wait `python -m comfyui_api wait --prompt-id <id> --timeout <s>`(收割 outputs)。The shared helper SHALL NOT be specialized for audio — capability dispatch happens entirely through the 4-dict `_validate_outputs` table
...
**Then** the returned `AudioCandidate.duration_seconds is None` and `.sample_rate is None`; the `format` field is correctly set; `metadata` contains the five required `comfy_*` keys(`comfy_manifest`, `comfy_params_snapshot`, `comfy_capability`, `comfy_original_filename`, `comfy_prompt_id`)plus `comfy_subprocess_run_metadata`; `tests/unit/test_comfy_subprocess.py::test_generate_audio_metadata_best_effort_when_comfy_does_not_emit` fences this
...
**Given** caller invokes `worker.generate(spec={"comfy_workflow": "x", "comfy_params": {"seed": 42}, ...}, num_candidates=3, seed=100)`
**When** worker runs 3 subprocess calls (per-candidate loop)
**Then** subprocess `i` receives `--params` JSON with `seed = 100 + i` (i ∈ {0, 1, 2})
**And** the inner `comfy_params.seed = 42` value MUST NOT survive into the subprocess invocation
...
**And** behavior is functionally identical to pre-fix (since `setdefault` and direct overwrite both yield base_seed when num_candidates=1)
...
**Given** a Step with `provider_policy.prepared_routes` containing `ResolvedRoute(model="comfy/local", ...)` and an executor injected with a `FakeComfyWorker` (which is what `framework.run` does for image fallback)
**When** `executor.execute(ctx)` runs and dispatches via `_should_use_worker_path() == True` and `_generate_via_worker()`
**Then** the resulting image `Artifact.producer.provider` MUST equal `"comfy_agent_cli"`
**And** the bundle Artifact's `producer.provider` MUST also equal `"comfy_agent_cli"`
...
**Given** a Step with `prepared_routes` containing `comfy/local-mesh` and a mesh executor injected with `HunyuanMeshWorker` or `FakeMeshWorker`
**When** `executor.execute(ctx)` dispatches via `_should_use_comfy_worker_path() == True` and `_generate_via_comfy_worker()`
**Then** the mesh `Artifact.producer.provider` MUST equal `"comfy_agent_cli"` and `model` MUST equal `"comfy/local-mesh"`
**And** the mesh cost model MUST be computed against `model="comfy/local-mesh"` (not the injected mesh worker's name)
**And** `result.metrics["worker"]` MUST equal `"comfy_agent_cli"`

## Scenario: Remote / fake mesh path attribution unchanged

**Given** a Step with `prepared_routes` containing only remote routes (e.g. `hunyuan/hy-3d-3.1`) and no `comfy/local-mesh`
**When** `executor.execute(ctx)` runs via the regular `self._worker.generate(...)` path (NOT comfy branch)
**Then** `Artifact.producer.provider` MUST equal `self._worker.name` (regression-safe — remote mesh path attribution is unchanged)
...
**Given** a `ComfyAgentWorker` with `comfy_output_root` resolved to `<root>` and a subprocess returning `outputs.images: ["<outside>/leak.png"]` where `<outside>` is not under `<root>`
**When** `worker.generate(spec=..., num_candidates=1)` runs and `_run_once` reaches the per-path loop
**Then** the worker SHALL raise `WorkerUnsupportedResponse` with a message containing `"outside comfy_output_root"` and a hint about `FORGEUE_COMFY_OUTPUT_ROOT`
**And** SHALL NOT call `shutil.copy2` or `read_bytes()` on the out-of-root path

## Scenario: mesh output path outside comfy_output_root is rejected

**Given** a mesh-mode `ComfyAgentWorker` and a subprocess returning `outputs.glb: ["<outside>/leak.glb"]`
**When** `worker.generate_mesh(...)` reaches the per-path loop
**Then** the worker SHALL raise `WorkerUnsupportedResponse` matching `"outside comfy_output_root"`

## Scenario: audio output path outside comfy_output_root is rejected

**Given** an audio-mode `ComfyAgentWorker` and a subprocess returning `outputs.audio: ["<outside>/leak.flac"]`
**When** `worker.generate_audio(...)` reaches the per-path loop
**Then** the worker SHALL raise `WorkerUnsupportedResponse` matching `"outside comfy_output_root"`

## Scenario: real ComfyUI install layout passes containment

**Given** a production install where `scripts_dir = D:/AI/ComfyUI/scripts` (heuristic root resolves to `D:/AI/ComfyUI`) and ComfyUI writes outputs to `D:/AI/ComfyUI/outputs/main/<date>/<project>/<file>`
**When** the worker reads any `outputs.images / .glb / .audio` path
**Then** the containment check SHALL PASS (the path is under the resolved root) without requiring `FORGEUE_COMFY_OUTPUT_ROOT` env var
**And** L2 live smoke is verified (FLAC artifact 1.17 MB persisted end-to-end at `artifacts/2026-05-04/audio_smoke_path_containment_l2/...`)
...
- `_run_once_video(...)` internal: invokes `_run_comfy_prompt(spec_for_call, timeout_s)`(自 `comfy-detach-wait-adoption` 2026-06-11;底层走 `_invoke_comfy_cli_once` detach+wait 两段式,替代原 `_run_subprocess_and_validate`)to obtain `outputs` dict (three-tier `_validate_outputs` already enforced for video); iterates `outputs.video` (absolute-path string list per ComfyUI agent CLI `runner.py::extract_outputs` source contract — same protocol as audio); for each path:
  - `src = Path(abs_path)`
...
**Given** `ComfyAgentWorker._capability == "video"` and `_run_comfy_prompt`(detach+wait 两段式,自 `comfy-detach-wait-adoption` 2026-06-11)returning `outputs.video = ["<abs_path>/wan21_1.3b_5sec_00001.mp4"]`; the file at that path exists, is not a symlink, and contains valid mp4 bytes (magic `b"ftyp"` at offset 4)
