...
- 不卸载或删除全局 forge 插件。
- 不由 Codex 执行任何删除文件操作。
- 不重写历史 archive 内容里的原始叙述,只迁移位置并更新 live 文档引用。

## 事实基线

- Codex 当前可见插件未包含 forge 插件。
- 项目当前 live 文档仍把 forge 写作主工作流入口。
- 项目 `forge/` 目录承担三类职责:
...
- `README.md`: `AI Workflow / forge` 改为 `AI Workflow / Superpowers`。
...
1. 路径验证:确认新路径文件存在,并且旧 live 引用已改到新路径。
