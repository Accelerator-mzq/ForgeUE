...
| FR-WORKER-011 Audio worker baseline + ComfyUI audio capability(自 v1.7)| `audio_worker.py::AudioWorker/AudioCandidate` + `comfy_worker.py::generate_audio` + `generate_audio.py::GenerateAudioExecutor` + `test_audio_worker.py`(6 fence) + `test_comfy_subprocess_audio.py`(19 fence) + `test_generate_audio_comfy.py`(14 fence) + `test_failure_mode_map.py`(audio +6) + `test_probe_framework.py`(audio +2) + `test_model_registry.py`(audio +2) | ✅ Level 0/1 通过;⚠️ Level 2 evidence DEFERRED post-archive(ComfyUI 0.9.2 user-authored workflow JSON `SaveAudioMP3` 缺 `quality` required input 上游 bug;沿 Phase 1 mesh L2 partial precedent;reproduction 步骤见 `openspec/changes/comfy-agent-cli-audio-adoption/notes/live_smoke_audio_blocked_20260503.md`) |
...
| v1.7 | 2026-05-03 | ComfyUI audio capability adoption(OpenSpec change `comfy-agent-cli-audio-adoption`,SRS TBD-009 Phase 2 + TBD-002 lift):新增 audio worker baseline `audio_worker.py::AudioWorker/AudioCandidate` + 异常树(`AudioWorkerError/Timeout/UnsupportedResponse`);新增 `runtime/executors/generate_audio.py::GenerateAudioExecutor`((StepType.generate, audio.t2a) ExecutorRegistry 注册);ComfyAgentWorker 4-dict capability dispatch 扩 audio(REQUIRED=audio key,REJECTED={images,glb,video},AUXILIARY=空);新虚拟 model id `comfy/local-audio` + alias `audio_local`(SRS FR-MODEL-007 第 11 alias);FailureModeMap 新增 `audio_worker_timeout` / `audio_worker_unsupported`(均 abort_or_fallback,沿 mesh_worker_* 同模式);DryRunPass.`_check_comfy_reachability` gate set 扩入 `comfy/local-audio`;新 example `examples/comfy_local_smoke_audio.json`;新 probe `probes/provider/probe_comfy_audio.py`(opt-in `FORGEUE_PROBE_COMFY_AUDIO=1`)。新增 fence 文件 `test_audio_worker.py`(6)+ `test_comfy_subprocess_audio.py`(19)+ `test_generate_audio_comfy.py`(14);delta `test_failure_mode_map.py`(audio +6)+ `test_probe_framework.py`(audio +2)+ `test_model_registry.py`(audio +2)。Plan-stage 1 轮 codex design review + 7 轮 codex plan review;Apply-stage 13-commit chain。**L2 evidence DEFERRED post-archive**(ComfyUI 0.9.2 user-authored workflow JSON `SaveAudioMP3` 缺 `quality` required input 上游 bug;沿 Phase 1 mesh L2 partial precedent;reproduction 步骤见 change `notes/live_smoke_audio_blocked_20260503.md`)。FR-WORKER-011 加入验收;TBD-002 ❌→⚠️ baseline;§8.1 自动化验收基线 1184 → **~1230**(+~46 audio fence) | ForgeUE Team |
