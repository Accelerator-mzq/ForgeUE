...
| FR-ENGINE-003 | Unreal adapter 应保持既有 `manifest_only` 行为,继续产出 `UEAssetManifest` / `UEImportPlan` / `Evidence`,并保留 `engine_scripts/unreal/run_import.py` 文件契约 |
...
| FR-WORKER-002 | 系统应支持 Hunyuan 3D worker(tokenhub 协议),生成 `mesh.gltf` / `mesh.fbx`;`.fbx` 候选必须做 dependency-free self-containment gate:若 `FileName` / `RelativeFilename` 周围出现常见贴图/媒体 sidecar 扩展名,普通模式 raise `MeshWorkerUnsupportedResponse` 触发 URL fallthrough / fallback;`spec.texture=False AND spec.pbr=False` 的 geometry-only 模式可接受并标记 `missing_materials=True` |
...
| NFR-PORT-003 | UE 侧(`engine_scripts/unreal/`)应最小化依赖,仅 `import unreal` |
...
| ADR-011 | Workflow runtime enforcement layer — D-WorktreeEnforce + D-DirectWorktreeRefinement + D-SkillCascadeCheck + D-RoundFixContinuity + D-TaskGranularityDeclaration + D-ParallelDispatch + D-PreflightProtocol + D-ProtocolVersionMigration 合记 ADR — 把 declared-only Layer 6 cascade dependency / worktree isolation / round 2 continuity / task granularity 转为 advisory enforcement(命令模板显式步骤 + LLM 自报 frontmatter declaration + finish_gate audit) **[D-WorktreeEnforce mandatory worktree 部分 Superseded by ADR-013;archived evidence pass-through;`_check_worktree_path` 改 mode-conditional advisory]** | 沿 ADR-010 自主路径协议进一步收敛 controller drift:enhance-workflow-automation change 实证 Layer 6(controller 漏 invoke `using-git-worktrees` SKILL)是真实 incident,不止理论风险。本 ADR 实装 4 runtime fence(`_check_skill_cascade` / `_check_round_fix_continuity` / `_check_task_granularity` / `_check_worktree_path`,protocol gate `runtime_enforcement_protocol_version: v1`)+ 1 个新命令(`/forgeue:change-apply-parallel`,invoke `superpowers:dispatching-parallel-agents` SKILL,借用 pattern 用于 implementation parallel dispatch;controller 显式判定 task 独立后路由,`task_independence_assertion: true` + `task_files_disjoint: [<file-set>...]` 字段;命令前自动 verify file overlap → 任一交集 abort)+ 8 个 SKILL-invoke 命令模板 Preflight section(`change-apply-{subagent,parallel}` 含 3 段 / `change-apply-direct` 含 2 段沿 D-DirectWorktreeRefinement 不强制 worktree / 5 个 SKILL-invoke 命令含 Skill Cascade 段)+ 1 个新工具(`tools/forgeue_skill_cascade_check.py` 384 行 stdlib only;静态扫 SKILL.md `## Integration` 段验证 dependency 全 invoke;8 root probe 链:CLI flag / env var / repo-local / Anthropic plugin cache / Codex / `${CODEX_HOME}` / `.agents/skills`)。Active forgeue 命令数从 9 → 10。**advisory not deterministic limitation**(R6;codex round 1 F1/F2/F3 揭示):markdown 命令模板 step + LLM 自报 frontmatter declaration + finish_gate audit 全部 advisory(controller 跳过 markdown step 时 subagent 已修改 + finish_gate 是 archive 时才扫,无法 abort);真 deterministic enforcement 留 follow-on `enhance-workflow-automation-executable-enforcement`(W1 executable preflight wrapper + machine-generated receipt JSON / W2 actual changed-files diff overlap detection / W3 dispatch ledger 命令层 wrapper)。**D-DirectWorktreeRefinement**(2026-05-05 user 拍板):`change-apply-direct` 沿 archived `2026-05-04-adopt-subagent-driven-development` D-Worktree-Detail 第 5 项**仍跑在主 worktree**(direct 是 < 3 micro-task 轻量 fallback,worktree 创建 ~10-20s 开销不划算);drift writeback commit `15ae851`。实装见 OpenSpec change enhance-workflow-automation-runtime-enforcement(2026-05-05) **[Retired by `retire-parallel-and-worktree-fully` 2026-05-06;wide retire all worktree / parallel / dispatch ledger ForgeUE-level enforcement layers;archived ADR-011/012/013 evidence pass-through 兼容]** |
| ADR-012 | Workflow executable enforcement layer v2 — 升级 ADR-011 v1 advisory not deterministic 为 v2 executable enforcement layer。D-W1-ReceiptSchema(wrapper 自管 worktree + 13-field receipt JSON)+ D-W2-OverlapDetection(parallel actual diff `git status --porcelain=v1` precondition + `git diff -z` + `git ls-files --others --exclude-standard -z` 合集 + 自动降级 sequential)+ D-W3-LedgerFormat(JSONL append-only ledger + 6 VALID_ROLES enum)+ D-W3-WrapperImpl(post-dispatch capture 真实 agent_id;关闭 round 1 synthetic UUID 漏洞)+ D-DispatchWrapperBoundary(LLM 不能直接写 receipt / ledger;wrapper-only write)+ D-DegradationPath(overlap 自动降级 sequential 无 user prompt)+ D-FrontmatterSchemaExtension(`runtime_enforcement_protocol_version: v2` + 7 v2 字段含 advisory 标注 origin)+ D-W4-IntegrationGate(P5.5 v2 e2e fixture archive 必过 gate;P10.0 二次确认)+ D-DogfoodGap(本 change evidence 仍 v1 advisory) **[D-W1-ReceiptSchema mandatory invocation 部分 Superseded by ADR-013 worktree mandatory parts;wrapper 改 opt-in;archived evidence pass-through]** | 沿 ADR-011 R6 limitation 关闭 codex round 1 F1/F2/F3 deferred gap。实装 2 新工具:`tools/forgeue_preflight_wrapper.py`(584 LOC + 18 fence;wrapper 自管 isolated worktree + 13-field receipt + cwd realpath 校验 + exit codes 0/5/6/7)+ `tools/forgeue_dispatch_ledger.py`(150 LOC + 12 fence;append/verify subcommand + 7 字段);`forgeue_finish_gate.py` v2 升级 +367 LOC + 524 LOC test(4 v2 fence:`_check_worktree_path_v2` receipt cross-check + `is_isolated_worktree: true` / `_check_round_fix_continuity_v2` ledger agent_id 集合 cross-check / `_check_file_overlap_actual` parallel only / `_check_dispatch_ledger` inline ledger verify);protocol gate `runtime_enforcement_protocol_version: v1/v2` dispatch matrix(legacy / v1 / v2 三路;v2 = v1 + additional checks 严格于 v1;archived runtime-enforcement evidence 兼容)。Active forgeue stdlib-only 工具数从 7 → 9。**Subagent Discipline Layer 2 wiring**:`/forgeue:change-apply-{subagent,parallel}` Preflight 段 MANDATORY invoke sister skill `subagent-driven-discipline`(controller-side 40% scenario judgment + Trigger Type Matrix retrospect)。**F2/F3 deferred 到 follow-on `enhance-workflow-automation-ledger-binding`**:wrapper-bound dispatch(F2 round 1 partial inline + deferred 真 wrapper 拦截)+ cryptographic ledger signing(F3 round 1 inline 降级 advisory 标注 + deferred 真 HMAC 防伪造);advisory limitation 暴露在 evidence frontmatter `pre_dispatch_metadata: advisory` + `ledger_forgery_resistance: advisory` 字段。Pre-P0 codex round 1 design review(5 high finding 全 accepted-codex,3 inline + 2 deferred)+ round 2 plan review(4 finding 全 inline plan-stage drift)。实装见 OpenSpec change enhance-workflow-automation-executable-enforcement(2026-05-05) **[Retired by `retire-parallel-and-worktree-fully` 2026-05-06;wide retire all worktree / parallel / dispatch ledger ForgeUE-level enforcement layers;archived ADR-011/012/013 evidence pass-through 兼容]** |
| ADR-013 | Restore Superpowers Worktree Consent Gate — D-RestoreConsentGate(命令模板 OPT-IN narrative + MUST invoke `Skill(superpowers:using-git-worktrees)` 沿 upstream cascade)+ D-ConsentOutcomeStateMachine(codex round 1 F2+F3 writeback:`worktree_consent_outcome` enum + `worktree_mode` enum 必填)+ D-AlreadyIsolatedInvariant(codex round 2 F2 writeback:`already_isolated` 必须 mode ∈ {skill,wrapper}_worktree + `worktree_path != main_repo`)+ D-ParallelDeclineFallback(codex round 1 F1 writeback:`/forgeue:change-apply-parallel` `declined + in_place` → 自动降级 sequential)+ D-WrapperDeprecate(`tools/forgeue_preflight_wrapper.py` 标 deprecated 但 functional)+ D-WrapperBugFixInScope(codex round 2 F3 writeback:W7-a wrapper bug fix 拨入本 change scope)+ D-CrossArchiveADRSupersede(SRS ADR-011/012 cross-reference 标 superseded by ADR-013 worktree mandatory parts) **[Supersedes (worktree mandatory parts) ADR-011 D-WorktreeEnforce + ADR-012 D-W1-ReceiptSchema mandatory invocation;archived ADR-011/012 evidence 不动]** | 沿 user 拍板 worktree consent policy(implementation default decline / bug-fix opt-in)— ADR-011/012 累积 mandatory worktree 实质 override Superpowers upstream consent gate;本 change revert mandatory 部分,restore upstream Step 0 user-consent gate。实装 2 新 fence(`forgeue_finish_gate.py::_check_worktree_consent_outcome` enum + outcome × mode invariant + W6 already_isolated path != main repo / `_check_worktree_mode_consistency` mode-conditional path/receipt 字段共存)+ 既有 `_check_worktree_path` v1 + `_check_worktree_path_v2` 改 mode-conditional advisory(legacy archived evidence pass-through)+ `_WORKTREE_REQUIRED_COMMANDS` frozenset retire 为空 + W7-a wrapper bug fix(`_git_repo_root` 改用 `git rev-parse --git-common-dir`,关闭从 worktree 内调用时返 worktree 自身 → nested target → "Filename too long" 链锁失败漏洞;2 unit fence test 守门 regression)+ 2 命令模板 `## Preflight Worktree` section 重写 OPT-IN narrative + 5 fence test(3 改 + 2 加 + 1 sister-md sync drift inline-fix)+ sister skill `subagent-driven-discipline` v2.3 加 §3.5 Worktree Consent Policy + Pattern 2 §3.1 STRICT cwd verify rewrite + Case 3 P0+P1 retrospect(13 inline fix;Pattern A reinforced + Pattern B 新 sister-file fence test sync drift + Pattern C 新 fence design intent docstring gap + Pattern D 新 controller inline fix > round 2 dispatch threshold)+ §6 catalog 加 2 row + frontmatter v2.2 → v2.3 + worktree_consent_policy + consent_outcome_enum + consent_mode_enum + case_study_count 2→3。Pre-P0 codex round 1 design review(3 finding 全 accepted-codex)+ round 2 plan review(3 finding 全 accepted-codex,W7-a wrapper bug fix 拨入本 change scope)。**legacy 兼容**:archived ADR-011/012 evidence 不含 `worktree_consent_outcome` 字段 → 全 fence pass-through。实装见 OpenSpec change restore-superpowers-worktree-consent-gate(2026-05-06) **[Retired by `retire-parallel-and-worktree-fully` 2026-05-06;wide retire all worktree / parallel / dispatch ledger ForgeUE-level enforcement layers;archived ADR-011/012/013 evidence pass-through 兼容]** |
...
| 入口 | 用途 |
| --- | --- |
| `engine_scripts/unreal/run_import.py` | UE Python Console `exec(open('...').read())` 触发导入 |
...
| v1.9 | 2026-05-20 | forge change `executor-async-rewrite`(TBD-010 close):全部 11 executor 硬切 `StepExecutor.execute` ABC → `async def`;orchestrator 删 `asyncio.to_thread` 包装改原生 `await executor.execute(ctx)`;ComfyAgentWorker 从 `subprocess.run` 改 `asyncio.create_subprocess_exec` + per-loop `_comfy_submit_lock` + cancel 时 server-side `/interrupt` + terminate;新 module `src/framework/runtime/lifecycle.py`(`ExternalProcessLifecycle` ABC + `ComfyLifecycleManager` 三模式 `ensure_running` / `ensure_release` / `self_managed_session`);orchestrator 持有 lifecycle manager + `arun` try/finally `release(mode, reason)` + `Orchestrator.aclose()` disposal 钩子;cascade-cancel 真停 + `_CASCADE_DRAIN_TIMEOUT_S=30s` drain 明示失败;`comfy_lifecycle` 从 `none`-only 扩展到四值受理(集合外才 raise,解锁 TBD-010);DryRunPass.run 改 `async def` + `aprobe` classmethod(Fluid Pause #1 scope 扩大);`StepContext.lifecycle: ExternalProcessLifecycle | None = None` 字段加入。FR-RUNTIME-005 / FR-WORKER-001 更新;§7.3 TBD-010 ✅。6 轮 codex adversarial review 收敛(13 finding 全核验);L2 live evidence `docs/archive/forge_changes/2026-05-20-executor-async-rewrite/notes/live_smoke_lifecycle_20260520.md`;全量 1179 passed(baseline 1136 + ~43 新 fence)。 | ForgeUE Team |
...
| v1.10 | 2026-05-23 | Linear FOR-14 `metadata-corruption-detection`:新增 FR-LC-009,要求新 run 为 `_artifacts.json` 写伴生 `_artifacts.integrity.json`;resume 发现 integrity 文件时先校验 metadata sha256 / artifact_count / artifact_ids,失败抛 `ArtifactMetadataIntegrityError` fail-fast,legacy 无 integrity 文件保持兼容。 | ForgeUE Team |
...
| TBD-002 ✅ | Audio worker(远端 Audio worker 接入) | 已落地(FOR-26,2026-05-23):在既有 `AudioWorker` ABC / `GenerateAudioExecutor` / `audio.t2a` baseline 上,新增通用 `RemoteHttpAudioWorker` + `audio_remote` alias;URL + 可选 API key/model 由 `FORGEUE_REMOTE_AUDIO_URL` / `FORGEUE_REMOTE_AUDIO_API_KEY` / `FORGEUE_REMOTE_AUDIO_MODEL` 注入。MiniMax 真实使用需求已补 `MiniMaxMusicWorker` + `audio_minimax` alias,由 `MINIMAX_KEY` 启用。ElevenLabs / AudioCraft 原生厂商 adapter 后续仅在真实使用时独立评估 |
| TBD-003 | WS 鉴权 / 多租户 session | 接入 UI 时再设计 |
| TBD-004 ✅ | FBX self-containment 校验。FOR-28 已落地 dependency-free guard:扫描 FBX `FileName` / `RelativeFilename` 附近的常见贴图/媒体 sidecar 扩展名,普通模式拒绝并 fallthrough / fallback,geometry-only 模式接受并标记 `missing_materials=True`。 | 已落地(2026-05-23);完整 PyFBX / ufbx 结构解析仍非目标,后续仅在需要更高精度时另立 follow-on |
...
| TBD-012 ✅ | `repo-put-streaming-payload`(大文件 stream copy):Phase 1 已完成 `repo.put(source_path=...)` / FileBackend streaming copy / stream hash 底层能力;Phase 2 已由 Linear FOR-13 完成 worker candidate source-path 迁移。本地 ComfyUI image / mesh / audio / video worker 只读格式校验头并返回 `source_path`,四个 generator executor 优先走 `repo.put(source_path=...)`;`data: bytes` 保留给 fake / 远端 worker 与无 `source_path` 的兼容回退。 | 已落地(2026-05-22);远端 worker 若未来需要避免下载全量驻留,按各 provider 独立 follow-on 评估 stream-download-to-temp + `source_path` |
