...
| NFR-PERF-005 | 全量测试套件应在 15 秒内跑完(491+ 用例) | CI 节奏保证 |
...
| 编号 | 决策 | 理由 |
| --- | --- | --- |
| ADR-001 | ForgeUE **不做 UE 插件形态**,坚持"文件契约 + 薄 UE Python 代理" | UE 插件会绑定 Python 版本、阻塞 game thread、无法跑 491 单测、隔离网络合规卡死、多工程复用困难、开发环境门槛陡增;ForgeUE 80% 职责与 UE 无关 |
