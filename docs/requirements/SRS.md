...
| TBD-009 | ComfyUI agent CLI mesh / audio / video workflow 接入。**Phase 1 mesh 已落地**(OpenSpec change `comfy-agent-cli-mesh-audio-video-adoption` 2026-05-03,5 轮 codex review + Round 5 D10 source bytes 写到 ComfyUI input/ via `FORGEUE_COMFY_INPUT_DIR` env;mini-LoadImage 变体 manifest user-authored 解决模型权重问题;L2 evidence GLB 真实生成 3.5 MB)。**Phase 2 audio 已落地**(OpenSpec change `comfy-agent-cli-audio-adoption` 2026-05-03,8 轮 codex review = 1 design + 7 plan + G6 verification + G11 adversarial,`comfy/local-audio` virtual model id + `audio_local` alias + Stable Audio Open / ACE-Step manifest 接入;**L2 evidence FULL PASS**:用户授权改 ComfyUI workflow JSON 一行后,真实 FLAC 1.17 MB 落地 + magic bytes 验证 PASS,evidence `archive/2026-05-03-comfy-agent-cli-audio-adoption/notes/live_smoke_audio_20260503_full.md`)。Phase 3 video 仍 follow-on(blocked-on video 输出策略决策)| Phase 1 + Phase 2 已归档;Phase 3 待开新 change |
