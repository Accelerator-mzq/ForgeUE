...
| `test_comfy_subprocess.py` (自 v1.6;自 `executor-async-rewrite` 大幅扩充;FOR-13 delta;`comfy-detach-wait-adoption` delta) | ComfyAgentWorker 全 subprocess CLI contract | FR-WORKER-001(v1.6 修订), NFR-REL-005 | L1 | REQUIRED-args(project_id/artifacts_dir/lifecycle 3) + probe_sync(missing scripts_dir / module / 30s timeout / 非零 exit 4) + 7-class failure mapping(Missing param / out of range / not_in_list / non-JSON / missing outputs / TimeoutError / unrecognised 7) + argv shape(workflow+params+lifecycle+timeout / task.project_id 2)+ outputs handling(copy-to-tree / glb raise / audio raise / legacy workflow_graph rejected 4) + **async-subprocess**(asyncio.create_subprocess_exec + asyncio.wait_for 替代 subprocess.run 阻塞)+ **串行锁**(_comfy_submit_lock per-loop AsyncioLock 防并发提交乱序)+ **cancel server-side abort**(_abort_comfy_prompt /interrupt + proc.terminate 双保险)+ **sync shim**(generate_image/mesh/audio/video sync shim 经 asyncio 桥接)+ **aprobe** classmethod(async status probe + sync probe_sync 并存)+ lifecycle 集合外值 raise(4 合法值 ∪ 集合外 → WorkerUnsupportedResponse 替代原 !="none")+ FOR-13 image/mesh source_path 不全读 fence + **detach+wait 协议**(submit `--detach` + wait `--prompt-id` 两段式 + `_invoke_comfy_cli_once` 共享 helper 防 drift fence `test_all_four_run_once_methods_route_through_shared_cli_helper`)+ **cancel `--prompt-id`**(abort 参数化 ×2 + 协议 fence `test_run_uses_detach_submit_then_wait_protocol` + prompt_id metadata ×4(image/mesh/audio/video)+ prompt_id 缺失 unsupported + submit 段失败分类)+ **cancel-on-timeout**(error_code/wall-clock timeout → 先 cancel 再 raise ×2 + wait/submit 段取消归因 ×2,`_HangingFakeProcess` 工具) |
...
| `test_comfy_subprocess_audio.py` (自 v1.7;FOR-13 delta;`comfy-detach-wait-adoption` delta) | ComfyAgentWorker.generate_audio + _run_once_audio + 4-dict audio capability dispatch | FR-WORKER-001 / FR-WORKER-011 | L1 | capability guard(comfy/local-audio gate)+ outputs.audio missing → WorkerUnsupportedResponse + 扩展名 whitelist 拒(`flac`/`mp3`/`wav` only)+ magic bytes 二次校验(`fLaC` / `ID3` / MPEG sync / `RIFF`+`WAVE`)+ path trust-boundary(symlink / 非 file)+ AudioCandidate metadata 5 个 comfy_* 键 + per-candidate seed 偏移 + REJECTED keys(images/glb/video)互斥 + source_path 记录且不全读 payload + **comfy_prompt_id metadata fence(detach-wait)** |
...
| `test_minimax_image_adapter.py` (MiniMax image follow-on) | MiniMaxImageAdapter 原生 image_generation 协议 | FR-WORKER-011 / FR-MODEL-007 | L1 | 6 用例(native POST payload + Bearer header + n fan-out + subject_reference passthrough + missing image_base64 → ProviderUnsupportedResponse + base_resp 非 0 → ProviderError) |
...
| `test_unreal_contract_package.py` | `engine_bridge/unreal/contract/*` 当前入口 | FR-ENGINE-003, FR-UE-001 | L1 | Unreal contract package-boundary fence:当前主路径可导入、当前代码不依赖 legacy alias、Godot4Adapter 不依赖 Unreal contract |
...
| 文件 | 覆盖 | 对应需求 | Level | 关键用例 |
| --- | --- | --- | --- | --- |
| `test_ue_bridge.py` | `engine_bridge/unreal/contract/*` + `engine_bridge/unreal/adapter.py` | FR-ENGINE-003, FR-UE-001 ~ FR-UE-008 | L1,L2 | Unreal manifest-only contract、ManifestBuilder modality 映射、PlanBuilder depends_on、Permission Phase C 默认拒绝、inspect_project / asset_exists、validate_manifest 重复路径、evidence 原子追加 |
...
| 文件 | 覆盖 | 对应需求 | Level | 关键用例 |
| --- | --- | --- | --- | --- |
| `test_probe_framework.py` (`comfy-detach-wait-adoption` delta) | `probe_*.py` 的 lazy-init | NFR-MAINT-* | L3 | 无 API key 环境 import 不崩 + **cancel probe ×2(detach-wait)**(`test_probe_comfy_cancel_default_skip_without_optin` + `test_probe_comfy_cancel_no_import_side_effects`) |
...
| `tests/unit/test_probe_framework.py` (delta;`comfy-detach-wait-adoption` delta) | probe_comfy_audio.py 顶层零副作用 + opt-in skip(`FORGEUE_PROBE_COMFY_AUDIO != "1"` 默认 skip) + **probe_comfy_cancel cancel probe ×2** | FR-OBSERV-* | L0 | +2 用例(import 不触 hydrate_env / mkdir,opt-in skip 输出 SKIP marker) + **+2 用例**(`test_probe_comfy_cancel_default_skip_without_optin` + `test_probe_comfy_cancel_no_import_side_effects`,cancel probe 零副作用 + opt-in gate) |
...
| `tests/unit/test_model_registry.py` (delta) | comfy/local-audio virtual id + audio_local alias + remote/audio virtual id + audio_remote alias + minimax/music-2.6 virtual id + audio_minimax alias + minimax/image-01 virtual id + image_minimax alias + image_local 仍解析(回归保护) | FR-MODEL-001/007 | L0 | audio_local 解析为 [comfy/local-audio];FOR-26 audio_remote 解析为 `ResolvedRoute(model="remote/audio", provider_kind="http", kind="audio", pricing=None)`;audio_minimax 解析为 `ResolvedRoute(model="minimax/music-2.6", provider_kind="http", api_key_env="MINIMAX_KEY")`;image_minimax 解析为 `ResolvedRoute(model="minimax/image-01", provider_kind="http", api_key_env="MINIMAX_KEY")` |
...
| `examples/minimax_image_smoke.json` (MiniMax image follow-on) | MiniMax 原生 image smoke bundle(`image.generation` + `provider_policy.models_ref="image_minimax"`) | FR-WORKER-011 / FR-EXAMPLES | L1 offline | loader + DryRunPass 自动收;真实 MiniMax 调用需用户提供 `MINIMAX_KEY` |
...
- ForgeUE 生成走 `comfyui_api run --detach` + `wait --prompt-id` 两段式,取消走 `cancel --prompt-id`(wait 超时也先 cancel 再 raise),探活走 `status`(自 `comfy-detach-wait-adoption` 2026-06-11);框架托管 lifecycle(`ComfyLifecycleManager._spawn_serve/_spawn_stop`)同走 `comfyui_api serve/stop`(2026-06-11 迁移,fence `test_spawn_serve_uses_comfyui_api_serve` / `test_spawn_stop_uses_comfyui_api_stop`)
