...
| 原则 | 说明 |
| --- | --- |
| **测试即可执行规范** | 555 个 pytest 用例本身是测试规范,本文档不重复描述每个用例的断言细节,只建立索引与矩阵 |
...
| NFR-MAINT | 所有 L3 fence 守门 + 总用例数 555(基线 491 + Codex audit fence 29 + src-layout / router-obs 根因定位 fence 6 + TBD-006 视觉 review 图像压缩 fence 10 + mesh quota error 分类 fence 19) |
| NFR-PORT | CI 能在 Linux 跑(555 全绿,stub unreal 覆盖 P4) |
...
| 级别 | 标准 |
| --- | --- |
| 单元测试 | 100% 通过(555 用例,基线 491 + audit 29 + 后续 fence 35)|
| 集成测试 | P0–P4 + 5 场景全绿 |
| Fence 测试 | 每条守护修复不得回退 |
