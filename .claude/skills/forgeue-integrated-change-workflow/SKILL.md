...
ForgeUE Integrated AI Change Workflow 的中心化编排器。本 skill 是 9 个 `/forgeue:change-*` command(`change-{status,plan,apply-subagent,apply-direct,debug,verify,review,doc-sync,finish}`)的共享 backbone:统一架构 + 状态机 + 协议,每个 command 只引用本 skill,不重复定义。
...
**历史**:
- 2026-05-04 `adopt-subagent-driven-development` 起拆 `change-apply` → `change-apply-subagent` + `change-apply-direct`
- 2026-05-05 `enhance-workflow-automation-runtime-enforcement`(ADR-011)+ 2026-05-05 `enhance-workflow-automation-executable-enforcement`(ADR-012)+ 2026-05-06 `restore-superpowers-worktree-consent-gate`(ADR-013)+ 2026-05-06 `enhance-workflow-automation-ledger-binding` 引入 ForgeUE-level worktree / dispatch ledger / parallel dispatch 强制层
- **2026-05-06 `retire-parallel-and-worktree-fully` 整层 retire ADR-011/012/013 + ledger-binding**(沿 D-HardRetireScope wide retire);worktree 沿 Superpowers upstream `using-git-worktrees` SKILL 自家 consent gate;parallel dispatch 路径不再支持(沿 D-PostRetireParallelStrategy);命令矩阵 10 → 9
...
| `using-git-worktrees` | **OPTIONAL — controller 自由 invoke**(沿 retire-parallel-and-worktree-fully retire ForgeUE-level 强制层后,upstream 自家 Step 0 consent gate 是唯一 worktree 协议入口);default decline → main repo cwd | controller 在 dispatch 前可自由 invoke 决定是否 isolation;无 ForgeUE-level mandatory invoke / 无 receipt JSON / 无 `worktree_consent_outcome` evidence 字段 |
| `subagent-driven-development` | **default sequential for `/forgeue:change-apply-subagent`**(ADR-009 token-budget tracker informational) | 4× LLM 调用;per-task 4 类 evidence + `subagent_budget.log`;ADR-009 与 ADR-007 vendor API 双扣边界**根本不同**;controller-side 40% scenario judgment 见 sister skill `subagent-driven-discipline`(scenario taxonomy + cheap-model reliability prompt patterns + cross-scenario discipline + Trigger Type Matrix retrospect)|
...
## v1 Advisory Runtime Fence(沿 ADR-011 advisory baseline,retire-parallel-and-worktree-fully P2 后)

`tools/forgeue_finish_gate.py` 内 3 个 v1 advisory fence(沿 ADR-011 D-SkillCascadeCheck / D-RoundFixContinuity / D-TaskGranularityDeclaration;`_check_worktree_path` 已在 retire change P2 整删):
...
| `_check_round_fix_continuity` | D-RoundFixContinuity | `subagent_continuity` dict(round 1/2 implementer + reviewer ID 一致;v1 advisory string-equality only,无 ledger cross-check)|
| `_check_task_granularity` | D-TaskGranularityDeclaration | `task_granularity` ∈ {phase, per-file, sub-task} |

**Protocol gating**(D-ProtocolVersionMigration + retire-parallel-and-worktree-fully D-ActiveVsArchivedReplayBoundary):
- `runtime_enforcement_protocol_version: v1` evidence → 走 v1 advisory fence
- 缺 `runtime_enforcement_protocol_version` 字段(legacy)→ 全 fence pass-through(archived ADR-010 时期 evidence 兼容)
- **Active 路径 evidence + present-but-invalid value**(`v2` / `v3` / typo / null / empty / `v4`)→ BLOCKER `unknown_protocol_version`(active 路径 typo 不可静默 bypass)
- **Archived 路径 evidence + 任何 unknown value** → legacy pass-through(归档不动)

**Preflight section**(D-PreflightProtocol;命令模板内含):

| 命令 | Preflight Skill Cascade | Preflight Task Granularity |
|---|---|---|
| `change-apply-subagent` / `change-apply-direct` | ✓ | ✓ |
| `change-plan` / `change-debug` / `change-verify` / `change-review` / `change-doc-sync` | ✓ | — |
| `change-finish` / `change-status` | **N/A**(纯工具 / 只读) | — |

**evidence frontmatter v1 字段集**:`runtime_enforcement_protocol_version: v1` + `triggered_by_command` + `skill_cascade_audit` + `subagent_continuity`(round 2+ fix 一致性) + `task_granularity`(`change-apply-{subagent,direct}` 必填)。

`tools/forgeue_skill_cascade_check.py` 静态扫 SKILL.md `## Integration` 段验证 dependency 全 invoke。

完整规则见 `docs/ai_workflow/forgeue_integrated_ai_workflow.md` §C.7 v1 Advisory Runtime Fence。
...
- **ForgeUE 命令**(`/forgeue:change-*`,9 个;沿 retire-parallel-and-worktree-fully P3 整删 `change-apply-parallel.md`):编排 S2-S8 实施 / cross-review / Sync Gate / Finish Gate;**不**做 contract create/archive
...
- ForgeUE memory:`feedback_verify_external_reviews` / `feedback_no_silent_retry_on_billable_api` / `feedback_decisive_approval` / `feedback_ascii_only_in_adhoc_scripts` / `feedback_self_reference_overcaution` / `feedback_partial_vs_whole_retire_audit`
