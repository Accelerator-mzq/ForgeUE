...
| `subagent-driven-development` | **default sequential for `/forgeue:change-apply-subagent`**(ADR-009 token-budget tracker informational) | 4× LLM 调用;per-task 4 类 evidence + `subagent_budget.log`;ADR-009 与 ADR-007 vendor API 双扣边界**根本不同**;controller-side 40% scenario judgment 见 sister skill `forgeue-subagent-discipline`(model 矩阵 / cwd verify / cross-verify / cherry-pick recovery / inline fix vs round 2 / skip review boundary) |
