...
| `audio.t2a` capability_ref 注册可能与现有 capability_ref 命名空间冲突(`image.generation` / `image.edit` / `mesh.generation` / `text.structured` / `text.review` / `review.judge` / `ue_export` 等) | F1 round-1 + R3-A round-3 修订:**不**改 `loader.py` step-kind 表(此表不存在);grep `capability_ref` 在 [`framework.run`](src/framework/run.py) ExecutorRegistry registration 段 + [`base.py:75`](src/framework/runtime/executors/base.py#L75) `(step.type, step.capability_ref)` 查找逻辑 检查命名空间唯一性;follow-on `audio.audio2audio`(remix)不在本 change scope |
...
4. GenerateAudioExecutor 新建 + `framework.run` ExecutorRegistry 注册 `(StepType.generate, "audio.t2a")` entry + `_should_use_comfy_worker_path` + `_generate_via_comfy_worker`(R3-A round-3 修订:**不**改 `loader.py`)
