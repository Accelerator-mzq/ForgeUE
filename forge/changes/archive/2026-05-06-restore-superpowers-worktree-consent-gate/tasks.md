...
- [x] P0.1:Read 现有 `.claude/commands/forgeue/change-apply-subagent.md` + `.claude/commands/forgeue/change-apply-parallel.md`(archived ADR-012 P3 mandatory 版)
- [x] P0.2:`change-apply-subagent.md` `## Preflight Worktree` section 重写(沿 codex round 1 F3 writeback):
...
- [x] P0.3:`change-apply-parallel.md` 同款重写 + parallel-specific(沿 codex round 1 F1 writeback):
...
- [x] P0.4:`change-apply-direct.md` 不动(沿 archived `2026-05-04-adopt-subagent-driven-development` D-Worktree-Detail 第 5 项 + ADR-013 D-AllChangeApplyMainRepoDefault align)
- [x] P0.5:`tests/unit/test_forgeue_command_markdown.py` fence test 调整(沿 codex round 1 F1+F3 writeback):
...
- [x] P0.6:`pytest tests/unit/test_forgeue_command_markdown.py -v` 全绿
- [x] P0.7:`python -m pytest -q` 全套 regress 全绿(无回归)

## P1 — `forgeue_finish_gate.py` advisory fence(D-AdvisoryFenceMode)

- [x] P1.1:Read `tools/forgeue_finish_gate.py` `_check_worktree_path` v1 + `_check_worktree_path_v2` 现状
- [x] P1.2:`_check_worktree_path` v1 改 mode-conditional advisory(沿 codex round 1 F2 writeback):
...
- [x] P1.3:`_check_worktree_path_v2` 改 mode-conditional advisory:
  - 入口加 `worktree_mode` field present check:absent → return [](legacy 兼容)
...
- [x] P1.4:加新 fence `_check_worktree_consent_outcome` + `_check_worktree_mode_consistency`(沿 codex round 1 F2+F3 writeback + codex round 2 plan review F2 writeback):
...
- [x] P1.5:`tests/unit/test_forgeue_finish_gate.py` 调整 + 加新 fence test:
...
- [x] P1.6:`pytest tests/unit/test_forgeue_finish_gate.py -v` 全绿
- [x] P1.7:`tests/integration/test_v2_e2e_synthetic_change.py` 11 test review:
...
- [x] P1.8:`python -m pytest -q` 全套 regress 全绿

## P2 — wrapper deprecate(D-WrapperDeprecate)

- [x] P2.1:`tools/forgeue_preflight_wrapper.py` 模块顶部加 `__deprecated_note__` 字符串 + module docstring 加 "[DEPRECATED in default flow]" 标记
- [x] P2.2:`--help` argparse description 加 deprecation notice("Deprecated in default flow per ADR-013;remains functional for opt-in bug-fix iteration use case")
- [x] P2.3:`tests/unit/test_preflight_wrapper.py` 18 fence test 全 PASS(行为不变;只 docstring/help notice 加 — 现 test 不依赖 docstring 内容)
- [x] P2.4:`python -m pytest -q` 全套 regress 全绿

## P3 — sister skill subagent-driven-discipline v2.3 update(D-CrossCheckUpstreamCascade)

- [x] P3.1:`.claude/skills/subagent-driven-discipline/SKILL.md` Pattern 2 STRICT cwd verify 重写:
...
- [x] P3.4:frontmatter `version: 2.2 → 2.3`;`case_study_count` 不变(2);加 `worktree_consent_policy: default-decline-in-implementation` 字段
- [x] P3.5:fence test 校验 sister skill SKILL.md 含 "Worktree Consent Policy" 字符串(若有;沿 archived 同款 fence)

## P4 — backbone skill `forgeue-integrated-change-workflow` update(D-CrossArchiveADRSupersede)

- [x] P4.1:`.claude/skills/forgeue-integrated-change-workflow/SKILL.md` Superpowers 集成边界表 `using-git-worktrees` 行重写:
...
- [x] P4.2:Runtime Enforcement Protocol(ADR-011)段加 superseded note:"D-WorktreeEnforce mandatory 部分由 ADR-013 superseded — restore Step 0 user-consent gate;_check_worktree_path fence 改 advisory"
- [x] P4.3:Runtime Enforcement Protocol v2(ADR-012)段加 superseded note:"D-W1-ReceiptSchema mandatory invocation 部分由 ADR-013 superseded — wrapper 改 opt-in"
- [x] P4.4:加新 "ADR-013:Restore Superpowers Worktree Consent Gate" section(简短摘要 + cross-link 本 change archive path)

## P5 — 9 处文档同步(沿 archived ADR-012 P5 模式)

- [x] P5.1:`docs/ai_workflow/forgeue_integrated_ai_workflow.md` §C.7 Runtime Enforcement(ADR-011)+ §C.8 v2(ADR-012)加 superseded note + 加 §C.9 "ADR-013 Restore Superpowers Worktree Consent Gate"
- [x] P5.2:`docs/ai_workflow/README.md` §4.4-bis(ADR-011)+ §4.4-ter(ADR-012)加 superseded note + 加 §4.4-quater "Worktree Consent Gate Restored(ADR-013)"
- [x] P5.3:`docs/ai_workflow/forgeue_quickstart.md` S3→S4-S5 stage 更新 — wrapper 改 opt-in 描述 + decline-default 行为
- [x] P5.4:`CLAUDE.md` 工具清单 9 工具不变(wrapper 留)+ Runtime enforcement frontmatter 段 v1 + v2 字段 OPTIONAL 标注 + 加 ADR-013 段
- [x] P5.5:`README.md` ForgeUE Workflow 表 9 工具不变 + ADR-013 摘要段
- [x] P5.6:`AGENTS.md` 加 ADR-013 段(沿 ADR-011 / ADR-012 同款结构)
- [x] P5.7:`CHANGELOG.md` `[Unreleased]` 加本 change entry
- [x] P5.8:`docs/requirements/SRS.md` ADR-013 行 + ADR-011 + ADR-012 行加 `Superseded by ADR-013 (worktree mandatory parts)` cross-reference
- [x] P5.9:`docs/acceptance/acceptance_report.md` ADR-013 status 行(✅ 已实装)+ ADR-011 / ADR-012 status 行加 supersede note

## P6 — verify

- [x] P6.1:`python tools/forgeue_verify.py --change restore-superpowers-worktree-consent-gate --level 0` 全绿
- [x] P6.2:`--level 1` 全绿(L1 SKIP opt-in 沿 ADR-007)
- [x] P6.3:产 `verification/verify_report.md`(12-key audit frontmatter)

## P7 — codex S6 mixed-scope review

- [x] P7.1:`/codex:review --base main --scope branch` mixed-scope 评(default background)
- [x] P7.2:落 `review/codex_mixed_scope_review.md`(verdict + finding verbatim + Claude file:line verify + Cross-check Matrix + Resolution Plan)
- [x] P7.3:writeback finding(沿 simplified protocol)
...
- [x] `review/codex_verification_review.md`(reference verify_report)
- [x] `review/codex_adversarial_review.md`(reference Pre-P0 + mixed-scope)
- [x] `review/design_cross_check.md`(A/B/C/D 4 段)
- [x] `review/plan_cross_check.md`(A/B/C/D 4 段)

## P8 — 跳过 superpowers requesting-code-review(沿 archived ADR-012 P8 同款 SKIP)

- [x] P8.1:写 `review/superpowers_review.md` SKIP rationale stub

## P9 — Documentation Sync Gate

- [x] P9.1:`python tools/forgeue_doc_sync_check.py --change restore-superpowers-worktree-consent-gate` 静态扫(0 DRIFT)
- [x] P9.2:落 `verification/doc_sync_report.md` evidence
- [x] P9.3:任何 [DRIFT] 项 → 修复

## P10 — Finish Gate

- [x] P10.1:`python tools/forgeue_finish_gate.py --change restore-superpowers-worktree-consent-gate --no-validate` 跑(预期 P11 unchecked sole blockers)
- [x] P10.2:验证 12-key frontmatter 全填
- [x] P10.3:验证 cross-check `disputed_open: 0`
- [x] P10.4:验证 evidence 全部 `runtime_enforcement_protocol_version: v2`(沿 D-DogfoodSelfHostMode revised path A literal compliance;wrapper-managed worktree;每 evidence 含 `worktree_path` + `worktree_receipt_path` + `worktree_consent_outcome: accepted` + `worktree_mode: wrapper_worktree`;ADR-013 outcome/mode fence 在 P1 ship 后 future replay 自动 enforce)
- [x] P10.5:验证 writeback_commit 真实性
- [x] P10.6:验证 tasks.md P0-P10 全 [x](P11/P12 留 archive)
- [x] P10.7:`openspec validate restore-superpowers-worktree-consent-gate --strict` 全绿
- [x] P10.8:落 `verification/finish_gate_report.md`(自 finish_gate 自动生成)

## P11 — Archive(用户授权 fence #1)

- [x] P11.1:**用户授权确认**(D-AutonomyBoundary fence #1 不可逆)
- [x] P11.2:`openspec archive restore-superpowers-worktree-consent-gate --skip-specs --yes`
- [x] P11.3:手工 sync 3 MODIFIED Requirement to `openspec/specs/examples-and-acceptance/spec.md`(替换原 ADR-011/012 mandatory 版)
- [x] P11.4:`openspec validate examples-and-acceptance --strict` 全绿
- [x] P11.5:archive stub 加 cross_check fence-required frontmatter(沿 ADR-012 P11.5)
- [x] P11.6:`git rm -r openspec/changes/<id>/`(防 archived ADR-012 同款 duplicate bug)
- [x] P11.7:commit + push(用户授权 fence #1)

## P12 — 后置(可选)+ Follow-on tracking

- [x] P12.1:更新 `MEMORY.md` 加 ADR-013 摘要(沿 forgeue auto memory 协议)
- [x] P12.2:实战 dogfood 验证 — 下一个 active change 按 ADR-013 default decline 流程跑(controller 默认 main repo cwd;不 invoke wrapper)
...
- [x] P12.4 (follow-on tracking;user 拍板 2026-05-05 retrospect):**`analyze-superpowers-skills-openspec-integration-gaps`** — 6 个 Superpowers 技能与 OpenSpec / ForgeUE workflow 体系**适配缺口**系统分析(brainstorming / explore stage,先 scope discovery 再 fix):
  - **scope 的 6 技能**:
