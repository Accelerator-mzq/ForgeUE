...
- **NEW** `ExternalProcessLifecycle` 抽象基类(`ensure` / `release(mode, reason)` /
  ...
  持有 manager、经 `StepContext` 新字段下传,`arun` 用 `try/finally` 在**所有**退出
  路径调 `release(mode, reason)`(`reason ∈ {run_end, cascade, arun_cancel,
  arun_error, orchestrator_close}`),停不停由 manager 的 (mode, reason) 决策表定 ——
  `ensure_release` 任一 run 退出 reason 都拆(含未分类异常 `arun_error`);
