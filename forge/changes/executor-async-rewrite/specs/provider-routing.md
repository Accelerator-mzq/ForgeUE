...
> 引入的行为增量:ComfyAgentWorker async-subprocess、`comfy_lifecycle` 三模式解锁、
> `ExternalProcessLifecycle` + `ComfyLifecycleManager`,以及 framework-managed
> ComfyUI lifecycle 相关 Invariant / Non-Goal 的 MODIFIED / REMOVED。每条
> Requirement 首行标注 ADDED / MODIFIED / REMOVED。
...
## Requirement: ComfyAgentWorker cancel terminates the subprocess
...
(or `wait_for` timeout), the worker SHALL terminate the `comfyui_api` subprocess in a
`finally` block — `proc.terminate()`, then `proc.kill()` after a bounded grace period
if the process has not exited. The worker SHALL NOT leak the subprocess after the
awaiting task is cancelled.

## Scenario: Cancel during ComfyAgentWorker run terminates the comfyui_api subprocess
...
**And** the `finally` block calls `proc.terminate()` and, after the grace period, `proc.kill()` if still alive
**And** the `comfyui_api` subprocess does not survive as an orphan, and `CancelledError` re-raises out of the worker so the orchestrator's cascade-cancel path observes the real cancellation
...
`FORGEUE_COMFY_LIFECYCLE` env default) as one of four values: `"none"` (assume a
user-owned ComfyUI is already running — unchanged behavior), `"ensure_running"`
(start ComfyUI if not already up, then leave it running), `"ensure_release"`
(ensure_running semantics, plus stop the instance at run end / cancel if and only if
this framework started it), and `"self_managed_session"` (the framework owns one
ComfyUI process for the orchestrator-instance lifetime, reused across runs, torn down
at session end / cancel). Any value outside this four-element set SHALL be rejected
with `WorkerUnsupportedResponse`. The lifecycle for the three non-`none` modes SHALL
be carried out by an `ExternalProcessLifecycle` handle, NOT by `ComfyAgentWorker`
itself (per-step workers are constructed inline and cannot own a session-scoped
process).
...
## Requirement: ExternalProcessLifecycle abstracts a framework-managed external process
...
sister CLI. The abstraction exists so a second managed-subprocess provider (SRS
TBD-011 follow-on) can be added as a second implementation without changing the
orchestrator-side wiring.

## Scenario: ComfyLifecycleManager.release stops only the instance the framework started

**Given** `comfy_lifecycle: "ensure_release"` where `ComfyLifecycleManager.ensure` found ComfyUI already running (started by the user) and therefore did NOT set the "framework started it" flag
**When** the run ends and `ComfyLifecycleManager.release("ensure_release")` runs
**Then** the manager does NOT stop ComfyUI because the user owns it
**And** conversely, when the manager itself started ComfyUI, `release` runs `python -m factory_v3 stop`
...
subprocess" above; the obsolete scenario "Cancel during ComfyAgentWorker run does not
produce orphan processes" (which asserted the subprocess "continues to run in the
worker thread until ComfyUI finishes the request naturally") is removed with it.
...
- Unit: `tests/unit/test_comfy_subprocess.py`(扩 — async-subprocess spawn / cancel
  terminate / sync-shim 兼容)、`tests/unit/test_comfy_lifecycle.py`(新 —
  `ComfyLifecycleManager` 三模式 + "framework started it" 标志 + release 守卫)。
