...
from framework.core.enums import ArtifactRole, Decision, PayloadKind
...
class UnrealAdapter:
    """Unreal manifest-only export adapter."""
    ...
    async def export(self, ctx: StepContext, *, target: EngineTarget) -> ExecutorResult:
        # EngineAdapter 边界只接收通用 EngineTarget;UE 旧逻辑仍使用 UEOutputTarget。
        ue_target = target.to_ue_target()

        # shutil.copy2 是 blocking IO,用 asyncio.to_thread 包装
        readiness = inspect_project(ue_target)
        if not readiness.project_root_exists:
            raise RuntimeError(
                f"UE project_root does not exist: {ue_target.project_root}"
            )
        if not readiness.content_dir_exists:
            # Create Content/ — real UE projects ship it but empty uprojects may not.
            (Path(ue_target.project_root) / "Content").mkdir(parents=True, exist_ok=True)
        ...
        if verdict is not None and verdict.decision == Decision.reject:
            ...
            return self._emit_rejection(ctx, verdict=verdict, target=ue_target)
        ...
        # Drop files
        run_folder = Path(ue_target.project_root) / "Content" / "Generated" / ctx.run.run_id
        ...
        if not dry_run:
            ...
            for art in importable:
                ...
                drop_dir, target_filename = derive_drop_target(
                    art, target=ue_target, run_id=ctx.run.run_id,
                )
                ...
                file_drop_evidence.append(Evidence(
                    ...
                    target_object_path=target_fs.relative_to(
                        Path(ue_target.project_root)
                    ).as_posix(),
                ))
        ...
        rebased_artifacts = [
            self._rebase_artifact_source(a, run_folder, Path(ue_target.project_root))
            for a in manifest_target_artifacts
        ]
        manifest = build_manifest(
            run_id=ctx.run.run_id, target=ue_target, artifacts=rebased_artifacts,
            import_rules={
                "dry_run": dry_run,
        ...
        manifest_art = self._persist_manifest_artifact(
            ctx, manifest=manifest, target=ue_target,
