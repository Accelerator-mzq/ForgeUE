...
| **6** | ComfyUI agent CLI live smoke:默认 `lifecycle=none` 时先确保 ComfyUI server running(本机推荐 `python -m comfyui_api serve` 作为启停 helper,`stop` 停;`factory_v3 serve/stop` 仍是兼容入口),再跑 `examples/comfy_local_smoke*.json`;也可设 `ensure_running` / `ensure_release` / `self_managed_session` 由框架托管 | 真实 image / mesh / audio / video 产物;ForgeUE 生成走 `run --detach` + `wait --prompt-id` 两段式(自 `comfy-detach-wait-adoption` 2026-06-11;manifest 名不变,不再 inline `workflow_graph`;不再用 `--comfy-url`)|
