...
### 升级 v3 Cryptographic Ledger Binding(自 `enhance-workflow-automation-ledger-binding` change 起,2026-05-06)

ADR-012 v2 dispatch ledger 是 advisory(`ledger_forgery_resistance: advisory`):cmd_verify 仅校 schema-level wrapper_version + monotonic timestamp,LLM 完全可以 hand-edit ledger 行 / 写 well-formed forge / 删除中间一行,fence 全部通过。本 change 升级 v3 为 **cryptographic enforcement** — HMAC-SHA256 hash chain + key-rotation fail-closed + tail truncation 守门 + audit consistency 强 enum + strict 11-field schema + unknown protocol 守门 + archived replay 路径限定;15 D-decision(round 1+2+3 codex inline writeback):

- **HMAC chain** `tools/_forgeue_ledger_crypto.py`(internal helper,~400 LOC stdlib-only):canonical JSON 序列化(排除 hmac + 含 prev_hmac + sort_keys + UTF-8)+ HMAC-SHA256 over canonical;HMAC key 持久化到 `~/.claude/forgeue_ledger_key`(JSON 单文件,跨 change 共享,0o600 权限);6 lifecycle 状态(首次 init / 正常 load / 文件损坏 exit 7 / key_id mismatch active default fail-closed BLOCKER / archived replay opt-in WARN exit 6 / forge 同 ledger 内 key_id 不一致 BLOCKER)
- **v3 ledger schema** `tools/forgeue_dispatch_ledger.py` 升级:WRAPPER_VERSION 1.0 → 2.0;cmd_append 写 11 字段(原 7 + protocol_version + key_id + prev_hmac + hmac);cmd_append stdout 打印 `[LEDGER] line_count=<N> final_hmac=<hex>` 行(LLM 复制到 evidence frontmatter);cmd_verify ANY v3 信号 dispatch(任一行含 hmac/prev_hmac/key_id 字段 OR wrapper_version=2.0 OR protocol_version=v3 → trigger v3 strict 否则 v2 schema-only legacy);加 `--allow-archived-replay` flag(仅 ledger 路径含 archive/ segment 才 honor)
- **新增 4 v3 fence** `tools/forgeue_finish_gate.py`:`_check_runtime_enforcement_protocol_version_validity`(round 2 codex F2 inline writeback;unknown protocol value `v4`/typo/empty/null → BLOCKER `unknown_protocol_version`)+ `_check_archived_replay_path_boundary`(round 2 codex F1 inline writeback;active path + `ledger_archived_replay: true` → BLOCKER `archived_replay_path_violation`)+ `_check_ledger_terminal_proof`(round 1 codex F3 inline writeback;v3 evidence MUST 含 `ledger_line_count` + `ledger_final_hmac` + cross-check 实际 ledger 防 tail truncation)+ `_check_ledger_forgery_resistance_consistency`(round 1 codex F4 inline writeback;v3 ↔ cryptographic / v2 ↔ advisory 强 enum)
- **`_check_dispatch_ledger` v3 分支** + **`_runtime_enforcement_active` v3 fence inheritance**(round 4 codex `/codex:review --base main` P1 inline writeback;v3 evidence 继承 v1 fence skill_cascade / task_granularity / worktree_path 等 7 个 v1 fence;沿 `_VALID_PROTOCOL_VERSIONS = frozenset({v1, v2, v3})` 替代原 `version in (v1, v2)`);v3 ⊇ v2 ⊇ v1 fence inheritance invariant
- **第 5 类 named DRIFT detector** `tools/forgeue_change_state.py:detect_drift_archived_replay_path`(round 3 codex F3 inline writeback;active change(非 archive/)evidence 含 `ledger_archived_replay: true` = drift signal 早期检测,与 finish_gate fence 双重守门)
- **2 命令模板升级**(`change-apply-{subagent,parallel}.md`):evidence frontmatter v2 → v3(`runtime_enforcement_protocol_version: v3` / `ledger_forgery_resistance: cryptographic` / `ledger_line_count: <int>` / `ledger_final_hmac: <64-hex>` 必填);Step 10a 加 stdout `[LEDGER]` 解析指令 + main session 串行 append invariant
- **Threat model 边界**:catch unintentional controller drift / sloppy bypass / hand-edit ledger / 中间删行 / 删尾行 / schema violation / unknown protocol / archived replay misuse;**不**承担 LLM 主动恶意 forge(LLM 可读 `~/.claude/forgeue_ledger_key`)— 留 future `enhance-workflow-automation-os-keystore` follow-on 用 OS-level secret store
- **superseded follow-on**:archived `executable-enforcement` P12.8 `enhance-workflow-automation-v2-fence-hardening`(本 change round 1 codex F5 scope expansion 已合并 schema strict);P9.6 `enhance-workflow-automation-archived-replay-audit`(round 2 codex F1 inline writeback 已实施 RETIRED);3 new follow-on tracked:`enhance-workflow-automation-skill-tool-binding`(F2 wrapper-bound dispatch deferred)+ `enhance-workflow-automation-os-keystore`(R1 mitigation)+ `enhance-workflow-automation-ledger-append-lock`(round 3 codex F4 deferred)

完整规则见 [`docs/ai_workflow/forgeue_integrated_ai_workflow.md`](docs/ai_workflow/forgeue_integrated_ai_workflow.md) §C.10 + OpenSpec change `enhance-workflow-automation-ledger-binding/design.md` 15 D-decision。
