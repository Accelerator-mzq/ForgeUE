...
def _scan_evidence_by_type(change_dir: Path) -> dict[str, list[Path]]:
    """Walk all evidence subdirectories; group .md files by frontmatter ``evidence_type``.
    ...
    for sub in _common.EVIDENCE_DIRS:
        sd = change_dir / sub
        if not sd.is_dir():
            continue
...
def _validate_evidence_file(
    ...
    if actual_type == "verify_report":
        if "[FAIL]" in body and fm.get("aligned_with_contract") is True:
            blockers.append(
                Blocker(
                    type="verify_report_inconsistent",
                    detail="aligned_with_contract: true but body contains [FAIL]",
                    file=rel,
                )
            )
    return blockers
