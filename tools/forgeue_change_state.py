...
def infer_state(change_dir: Path, *, archived: bool) -> tuple[str, list[str]]:
    ...
    if verify_path.exists():
        ...
        # P7 review F-E: use shared helper that strips the autogenerated
        # ``- [FAIL]: 0`` count-summary line; otherwise every PASS report
        # appears to "contain [FAIL]" and S5 is never inferred (verified
        # in P7 self-test: state stuck at S4 with the autogenerated summary
        # line). Keep state-machine semantics aligned with finish gate's
        # verify_report self-consistency check (F-A).
        if not _common.verify_report_has_real_failures(verify_text):
            state = "S5"
            reasons.append("verification/verify_report.md present, no real [FAIL] step (S5)")
        else:
            reasons.append("verification/verify_report.md present but contains real [FAIL] step")
