...
The system SHALL extend `StepContext` (`src/framework/runtime/executors/base.py:16-24`) with a new field `run_dir: Path` that resolves to the absolute path `<artifact_root>/<run_id>/`, where `<artifact_root>` comes from `getattr(self.checkpoints, "_root", None)` on the Orchestrator (the same source `Orchestrator._post_step` line 627 already uses for `dump_run_metadata`). NOTE: `framework.run --artifact-root` already date-buckets the path by default (`artifacts/<today>/`, see `framework.run` line 111-115), so `run_dir` MUST NOT add another date segment — `framework.run` line 149 also uses `artifact_root / args.run_id` directly without an extra date bucket. The Orchestrator SHALL inject this field via `_compute_run_dir(run)` helper when constructing `StepContext` for each Step execution, so executors and workers that need to place files inside the project artifact tree (e.g. `ComfyAgentWorker` copying ComfyUI outputs from `D:/AI/ComfyUI/outputs/...` to `<run_dir>/comfy/`) have a single canonical source for the run-scoped artifact directory. The field SHALL satisfy the existing `<run_dir>/_artifacts.json` convention (`runtime-core/spec.md:37`) — i.e. `run_dir == ArtifactRepository`'s working directory for that run. **Implementation note (drift writeback from G3 commit 2)**: the field is implemented as `field(default_factory=lambda: Path("."))` — NOT strictly REQUIRED at the dataclass level — to keep ~25 existing `StepContext(...)` mock callsites in `tests/` working without breaking mass test patches. The production invariant that "Orchestrator MUST inject the real path" is preserved by `Orchestrator._compute_run_dir` always being called at the single construction site (`orchestrator.py` line 459 ish); the default `Path(".")` is a test-mock convenience only and SHALL NOT be observable in any production execution path. See `tests/unit/test_orchestrator.py::test_orchestrator_compute_run_dir_uses_checkpoints_root_no_extra_date` for the production injection invariant fence.
