...
| `src/framework/providers/workers/comfy_worker.py` | **Rewrite** | rename `HTTPComfyWorker` → `ComfyAgentWorker`;删 HTTP 全套(requests / `/prompt` / `/history` / `/view`);保留 `WorkerError` / `WorkerTimeout` / `WorkerUnsupportedResponse` / `ImageCandidate` / `FakeComfyWorker`;新实装 **keyword-only** `__init__(*, scripts_dir, run_id, project_id, artifacts_dir, python_exe=None, default_lifecycle="none")`(round 3 H3 fix:required 在 default 之前)+ **sync** `generate(spec, *, num_candidates, seed, timeout_s)`(适配 `ComfyWorker` ABC 同步接口;内部用 `subprocess.run(..., timeout=...)` blocking;G4 commit 3 实施 drift:round 2/3 spec 写 async submit + asyncio.run bridge 实际不需要,因为 ABC 是 sync — written-back-to-spec)+ **sync classmethod** `probe_sync(scripts_dir, python_exe, timeout_s=30)`(round 3 plan P2 fix)+ 内部 copy 到 `artifacts_dir/comfy/` + non-image outputs raise |
| `src/framework/providers/workers/__init__.py` | **Modify** | 删 `HTTPComfyWorker` import + export;加 `ComfyAgentWorker` import + export(G4 commit 3 cascade fix — drift writeback after import error revealed) |
| `src/framework/run.py` | **Modify** | 删 `HTTPComfyWorker` import;`comfy_base_url`(从 `--comfy-url` flag)分支 deprecated:打印 warning + fallback 到 `FakeComfyWorker()`;production ComfyAgentWorker 由 GenerateImageExecutor inline 构造(per F-B + G2 dispatch),不再在 framework.run wire(G4 commit 3 drift writeback — written-back-to-execution_plan-cascade) |
| `tests/unit/test_adapter_budget_clamp.py` | **Modify** | 删 HTTPComfyWorker import + 删 2 个 HTTP-specific budget clamp fence(协议层不存在了;subprocess.run timeout 直接守等价 invariant);保留 Tripo3D + LiteLLM fence(G4 commit 3 drift writeback) |
| `tests/unit/test_comfy_http_unsupported.py` | **Delete** | HTTP 协议已不存在;移除 121 行旧 fence(G4 commit 3 提前删,原 G7 commit 6 计划) |
| `src/framework/runtime/executors/generate_image.py` | **Modify** | `_resolve_spec` 读 `comfy_workflow` / `comfy_params` / `comfy_lifecycle` 三字段;旧 `workflow_graph` 命中 raise;`comfy_lifecycle` 非 `"none"` raise;加 `_should_use_worker_path(ctx)` 检测 `prepared_routes` 含 `model == "comfy/local"` 时返 True;加 sync `_generate_via_worker(ctx, spec, num, seed, timeout_s)` 直接调 SYNC ABC `worker.generate(spec=..., num_candidates=..., seed=..., timeout_s=...)`(G11 R4 writeback:G4 commit 3 实施 drift 把 worker 改 sync `generate` 匹配 ABC,无需 asyncio.run bridge)inline 构造 `ComfyAgentWorker` 从 env config + `ctx.run_dir` + `ctx.task.project_id` + `ctx.run.run_id`(round 3 H1+H3 fix) |
...
- `src/framework/runtime/orchestrator.py` — `to_thread` 包装 + cancel 不可达同步 `worker.generate(...)` 的语义不动(G11 R4 writeback:G4 worker drift 后实际是 sync `generate` 而非 async submit;但 to_thread 包装 sync executor 同样 cancel 不可达,D6 选 A 后接受 best-effort,TBD-010 后续 change 重写 executor 为 async)。**例外:** G11 R3 加 `_compute_run_dir` fail-fast(`_root is None` raise RuntimeError)以避免 sync ComfyAgentWorker 把产物写到 cwd 破坏 artifact tree 自包含性
...
| G4 ComfyAgentWorker | `tasks.md#3.1` - `#3.6` | commit 3(`feat(comfy): replace ...`) | `comfy_worker.py` 重写,删 HTTP,加 subprocess + **keyword-only `__init__(*, scripts_dir, run_id, project_id, artifacts_dir, python_exe=None, default_lifecycle="none")`** + REQUIRED 字段 None 校验 + lifecycle assert + **SYNC `generate` 匹配 ABC**(G4 implementation drift writeback:round 2/3 spec 写 async submit 实际改 sync `generate` 因为 `ComfyWorker` ABC 是 sync)+ **sync `probe_sync` classmethod**(round 3 plan P2 fix)+ copy + 后续 G11 R1 加 `encoding="utf-8", errors="replace"` + R2 加 symlink/PNG magic 校验 |
| G5 Executor + dryrun + worker dispatch | `tasks.md#4.1` - `#4.5` | commit 4(`feat(executor+dryrun): ...`)| `generate_image.py` 加 `_should_use_worker_path` + sync `_generate_via_worker` **直接调 sync `worker.generate(...)`**(G11 R4 writeback:G4 worker 已是 sync,无需 asyncio.run bridge);`dry_run_pass.py` 加 model id-based gate **直接 sync 调** `ComfyAgentWorker.probe_sync(...)`(round 3 plan P2 fix:NOT `asyncio.run(probe(...))`) |
...
| Cancel 不可达 sync worker.generate(在 to_thread 包装内) | lifecycle=none 下 best-effort 可接受 | 实测发现孤儿进程 → STOP,触发 TBD-010 提前 |
