...
- **THEN** the executor calls `_generate_via_worker(ctx, spec)` (a new SYNC method); inside `_generate_via_worker`, the executor wraps the async worker call with `asyncio.run(...)`: `result = asyncio.run(self._aworker_call(ctx, spec))` where `_aworker_call` is an `async def` helper that constructs `ComfyAgentWorker(*, scripts_dir=..., run_id=ctx.run.run_id, project_id=ctx.task.project_id, artifacts_dir=ctx.run_dir, python_exe=..., default_lifecycle="none")` (keyword-only signature per H3 fix; required args first per Python rules) and `awaits` `worker.submit(spec, timeout_s=...)`; `_generate_via_router` is NOT called for this step; `router.image_generation(prompt, ...)` is NOT invoked; the `asyncio.run(...)` bridge mirrors the pattern at `generate_image.py:295`
...
- THEN the executor takes the worker dispatch branch and constructs `ComfyAgentWorker` inline from environment config (`FORGEUE_COMFY_*`) + `ctx.run_dir` + `ctx.task.project_id` + `ctx.run.run_id` (keyword-only signature per H3 fix); it calls `result = asyncio.run(self._aworker_call(ctx, spec))` to bridge the synchronous executor to the async worker (mirrors `_generate_via_router` pattern at `generate_image.py:295`); `CapabilityRouter.image_generation` is NOT called for this step; LiteLLM's wildcard never sees `comfy/local`. This is a NEW dispatch pattern for ForgeUE — distinct from mesh's pattern (b) which uses constructor injection
