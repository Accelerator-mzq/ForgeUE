...
| 路径 | 类型 | 责任 |
|---|---|---|
| `tools/forgeue_preflight_wrapper.py` | 新建 | W1 preflight wrapper:创建 / 验证 worktree + 内嵌 cascade check + 写 receipt JSON 到 `<change>/preflight_receipts/<id>.json` |
...
| `tests/unit/test_preflight_wrapper.py` | 新建 | W1 fence 14 个测试 |
...
## Phase Map(对应 tasks.md 锚点)

| Phase | tasks.md 锚点 | scope | 依赖 | 说明 |
|---|---|---|---|---|
| Pre-P0 | [Pre-P0](../tasks.md#pre-p0self-host-bootstrap沿-d-selfhost-模式本-change-实施期间-sequential-dispatch因为-w1-wrapper-还没-ship无法用-w2-actual-overlap-安全-parallel) | codex round 1 challenge + writeback | 已 4 制品 valid | 本会话当前阶段(Step 5/6/8/9) |
| P0 | [P0](../tasks.md#p0--toolsforgeue_preflight_wrapperpy-新建--测试-fencew1) | W1 wrapper + 14 fence test | Pre-P0 closed | 见 micro_tasks.md §P0 |
...
| P3 | [P3](../tasks.md#p3--forgeuechange-apply-subagentparallel-命令模板加-wrapper-invocation-step) | 命令模板 wrapper + ledger + actual diff + 6 fence test | P0 + P1 + P2 done | 见 micro_tasks.md §P3 |
...
| P6-P12 | [P6](../tasks.md#p6--verify) ~ [P12](../tasks.md#p12--后置可选--follow-on-tracking) | verify / codex review / superpowers skip / doc sync gate / finish gate / archive / follow-on | P0-P5 done | 沿 archived runtime-enforcement 同款编排,详见 tasks.md |
