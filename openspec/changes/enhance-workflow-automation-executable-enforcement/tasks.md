...
- [ ] Pre-P0.1:`/codex:adversarial-review` round 1 挑战 D-W1-ReceiptSchema / D-W2-OverlapDetection / D-W3-LedgerFormat / D-DispatchWrapperBoundary / D-DegradationPath / D-FrontmatterSchemaExtension / D-DogfoodGap / D-W3-WrapperImpl 8 个 D-decision + Open Questions OQ-1/2/3/4
...
- [ ] P0.1:Read 现有 `tools/forgeue_skill_cascade_check.py` 了解 stdlib 风格 + argparse 协议
- [ ] P0.2:加 `tools/forgeue_preflight_wrapper.py`(stdlib only;F1 round 1 codex inline writeback 后 wrapper 自管 worktree):
...
- [ ] P0.3:加 `tests/unit/test_preflight_wrapper.py` fence(预计 18 个;F1 round 1 inline 加 4 个新 negative test):
...
- [ ] P0.4:`pytest -q tests/unit/test_preflight_wrapper.py` 全绿
- [ ] P0.5:`pytest -q` 全套 regress 全绿(无回归)

## P1 — `tools/forgeue_dispatch_ledger.py` 新建 + 测试 fence(W3)

- [ ] P1.1:加 `tools/forgeue_dispatch_ledger.py`(stdlib only):
...
- [ ] P1.2:加 `tests/unit/test_dispatch_ledger.py` fence(预计 12 个):
  ...
  - 1 CLI smoke
- [ ] P1.3:`pytest -q tests/unit/test_dispatch_ledger.py` 全绿
- [ ] P1.4:`pytest -q` 全套 regress 全绿

## P2 — `forgeue_finish_gate.py` 升级 4 fence + 协议 v2 dispatch + 测试

- [ ] P2.1:Read `tools/forgeue_finish_gate.py` 现状(P1/P2/P3 simplified protocol 之后 + 4 runtime fence v1 版)
- [ ] P2.2:在 `check_frontmatter_protocol` 入口加 `runtime_enforcement_protocol_version` dispatch:
...
- [ ] P2.3:升级 `_check_worktree_path` v2:
...
- [ ] P2.4:升级 `_check_round_fix_continuity` v2:
...
- [ ] P2.5:加 `_check_file_overlap_actual` 新 fence(W2):
...
- [ ] P2.6:加 `_check_dispatch_ledger` 新 fence(W3):
...
- [ ] P2.7:在 `check_frontmatter_protocol` 调用链插入新 fence(各独立 Blocker.type)
- [ ] P2.8:加 `tests/unit/test_forgeue_finish_gate.py` v2 fence 测试(预计 12 个):
...
- [ ] P2.9:加 `tests/unit/test_forgeue_finish_gate.py` protocol_version dispatch 测试(预计 4 个):
...
- [ ] P2.10:`pytest -q tests/unit/test_forgeue_finish_gate.py` 全绿
- [ ] P2.11:`pytest -q` 全套 regress 全绿

## P3 — `/forgeue:change-apply-{subagent,parallel}` 命令模板加 wrapper invocation step

- [ ] P3.1:Read 现有 `.claude/commands/forgeue/change-apply-subagent.md` + `change-apply-parallel.md`(P2 既有 Preflight section)
- [ ] P3.2:`change-apply-subagent.md` `## Preflight Worktree` section 升级:
...
- [ ] P3.3:`change-apply-parallel.md` 同款升级 + 新增 W2 actual diff 段(F4 round 1 codex inline writeback 后):
...
- [ ] P3.4:`change-apply-direct.md` **不动**(沿 D-DirectWorktreeRefinement;direct 路径无 wrapper 调用,无 ledger 写,evidence 仍 v1 advisory)
- [ ] P3.5:`tests/unit/test_forgeue_command_markdown.py` 加 v2 fence(预计 6 个):
...
- [ ] P3.6:`pytest -q tests/unit/test_forgeue_command_markdown.py` 全绿;`pytest -q` 全套 regress 全绿

## P4 — backbone skill SKILL.md 同步 W1/W2/W3 wrapper invocation 协议

- [ ] P4.1:`.claude/skills/forgeue-integrated-change-workflow/SKILL.md` 加 "Runtime Enforcement Protocol v2(本 change 引入)" 段:
...
- [ ] P4.2:命令清单表(沿 P2 模式)无新命令(W1/W2/W3 都是 wrapper 加强,不新增命令);Superpowers 集成边界表加 wrapper 拦截层说明
- [ ] P4.3:`tests/unit/test_forgeue_command_markdown.py`(SKILL.md 静态扫 fence)若有 → 加 W1/W2/W3 关键词 fence(可选,若 SKILL.md 不在 fence scope 则跳过)

## P5 — 11 处文档同步(沿 enhance-workflow-automation-runtime-enforcement P4 模式)

- [ ] P5.1:`docs/ai_workflow/forgeue_integrated_ai_workflow.md` §C 加 §C.8 "Executable Enforcement Layer v2"(W1 wrapper + W2 actual diff + W3 ledger + protocol_version v2 dispatch + 6 fence 表)+ DogfoodGap 段
- [ ] P5.2:`docs/ai_workflow/README.md` §4 加 §4.4-ter "Executable Enforcement v2"(沿 §4.4-bis 同款结构)+ 引 ADR-012
- [ ] P5.3:`docs/ai_workflow/forgeue_quickstart.md` S3→S4-S5 stage 加 wrapper 协议摘要(用户视角):"运行命令时自动生成 receipt + ledger,不需手动写 worktree_path"
- [ ] P5.4:`CLAUDE.md` `## OpenSpec 工作流` 段:
...
- [ ] P5.5:`README.md` ForgeUE Workflow 表:7→9 工具(加 W1/W3 wrapper);新增 ADR-012 摘要段
- [ ] P5.6:`AGENTS.md` 加 4 条 v2 enforcement 摘要(W1 receipt / W2 actual diff / W3 ledger / protocol_version v2 dispatch)
- [ ] P5.7:`CHANGELOG.md` `[Unreleased]` 加本 change entry(完整覆盖 ADR-012 + 6 fence + 2 wrapper + W2 actual diff + protocol v2 + commit SHA + 测试覆盖)
- [ ] P5.8:`docs/requirements/SRS.md` 加 ADR-012 行(沿 ADR-007/008/009/010/011 格式;含 8 D-decision 摘要 + DogfoodGap + W1/W2/W3 wrapper 协议)
- [ ] P5.9:`docs/acceptance/acceptance_report.md` 加 ADR-012 status 行(✅ 已实装,全条目对应 SRS ADR-012)
- [ ] P5.10:`docs/design/HLD.md` workflow tooling 段加 W1/W3 wrapper(沿 ADR-011 同款描述)+ protocol_version dispatch 简述
- [ ] P5.11:`openspec/specs/examples-and-acceptance/spec.md` — sync archive 时 auto-merge(本 task 不动;P10 archive 协议处理)

## P5.5 — v2 e2e integration test fixture(F5 round 1 codex inline writeback;archive 前必过 gate)

- [ ] P5.5.1:Read `tests/integration/test_p3_*.py` 了解 ForgeUE integration test 风格(tmp_path + 端到端 bundle)
- [ ] P5.5.2:加 `tests/integration/test_v2_e2e_synthetic_change.py`(沿 D-W4-IntegrationGate;预计 ~200-300 LOC):
...
- [ ] P5.5.3:`pytest -q tests/integration/test_v2_e2e_synthetic_change.py -v` 全绿(预计 8-12 test case)
- [ ] P5.5.4:`pytest -q` 全套 regress 全绿(无回归)

## P6 — verify

- [ ] P6.1:`python tools/forgeue_verify.py --change enhance-workflow-automation-executable-enforcement --level 0` 全绿
- [ ] P6.2:`--level 1` 全绿(L1 live-llm SKIP 沿 ADR-007 钱 fence 边界,默认 skip 不 block)
- [ ] P6.3:产 `verification/verify_report.md`(12-key audit frontmatter)

## P7 — codex S6 mixed-scope review

- [ ] P7.1:`/codex:review --base <main> --background --scope branch` mixed-scope 评(default background;沿 D-DefaultBackground)
- [ ] P7.2:落 `review/codex_mixed_scope_review.md`(verdict + finding verbatim + Claude file:line verify + B Cross-check Matrix + Resolution Plan)
- [ ] P7.3:writeback finding(沿 ADR-010 simplified protocol 单 commit pattern)
...
- [ ] `review/codex_verification_review.md`(reference verify_report + 待 mixed-scope review 完成)
- [ ] `review/codex_adversarial_review.md`(reference Pre-P0 round 1 + 待 mixed-scope review 完成 round 2)
- [ ] `review/design_cross_check.md`(reference Pre-P0 plan_cross_check;A/B/C/D 4 段)
- [ ] `review/plan_cross_check.md`(reference Pre-P0 plan_cross_check;A/B/C/D 4 段)
...
- [ ] P8.1:写 `review/superpowers_review.md` SKIP rationale stub(reviewed N layer review coverage matrix)

## P9 — Documentation Sync Gate

- [ ] P9.1:`python tools/forgeue_doc_sync_check.py --change enhance-workflow-automation-executable-enforcement` 静态扫(11 [REQUIRED] 全 touched_in_change 或 sync archive 时 auto-merge / 0 [DRIFT])
- [ ] P9.2:落 `verification/doc_sync_report.md` evidence(12-key frontmatter)
- [ ] P9.3:任何 [DRIFT] 项 → 0 DRIFT,无需修复

## P10 — Finish Gate

- [ ] **P10.0(F5 round 1 codex inline writeback;archive 前必过 gate)**:`pytest -q tests/integration/test_v2_e2e_synthetic_change.py -v` 全绿(沿 D-W4-IntegrationGate;v2 协议端到端实跑 + overlap 负例 + 全 6 fence + v1/legacy 回归);**fixture 不绿 → archive 阻断**
- [ ] P10.1:`python tools/forgeue_finish_gate.py --change enhance-workflow-automation-executable-enforcement --no-validate` 跑(预期 P11/P12 unchecked blocker 沿 self-host bootstrap 模式)
- [ ] P10.2:验证 12-key frontmatter 全填(全部 formal evidence 12-key)
- [ ] P10.3:验证 cross-check `disputed_open: 0`
- [ ] P10.4:验证 runtime enforcement frontmatter 字段:
...
- [ ] P10.5:验证 writeback_commit 真实性(Pre-P0 + 任意 drift writeback 全 git rev-parse 可解析)
- [ ] P10.6:验证 tasks.md 全 [x] 勾选(P0-P10 全 [x];P11 archive + P12 follow-on tracking 必然 [ ] 直到 archive 操作本身;沿 archived 同款 self-host bootstrap unchecked blocker)
- [ ] P10.7:`openspec validate enhance-workflow-automation-executable-enforcement --strict` 全绿(待 P11.4 sync archive 后再跑)
