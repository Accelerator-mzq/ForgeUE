...
- [x] 9.1 `python tools/forgeue_finish_gate.py --change fuse-openspec-superpowers-workflow --json` exit 0(2026-04-27:0 blockers post-fixup;exit 0 + 0 warnings;含 `openspec validate --strict` 通过)
- [x] 9.2 `verification/finish_gate_report.md` 落盘 + 所有 evidence frontmatter `aligned_with_contract: true`(或带 drift 标记 + reason ≥ 50 + design.md "Reasoning Notes" anchor;两份新 plan-stage thin evidence 走 disputed-permanent-drift 协议 + design.md §11.5 anchor 解析通过)
- [x] 9.3 `~/.claude/settings.json` 与 `~/.claude-max/settings.json` 实测均不含 `--enable-review-gate`(verified 2026-04-27)

### 9.5 P8 review 修复(post-finish-gate fixups,2026-04-27)

P8 close-out 跑 `forgeue_finish_gate.py --no-validate --dry-run`(`verification/finish_gate_report.md`)暴露 11 blockers,2 类:类型 A(2 blockers,plan-stage codex/cross-check evidence missing — Pre-P0 / P0 阶段 plan 与 design stage 合并未独立产出)+ 类型 B(9 blockers,§9-§11 self-stage tasks_unchecked — finish_gate 是它们的 gate,要求先 [x] 是 chicken-and-egg)。沿 ForgeUE memory `feedback_decisive_approval`,P8 onboarding doc 已论证 + 推荐方案 A.1(类型 A)+ B.1(类型 B),用户绿灯 self-host 推进,3 fix 在本 P8 阶段一次性闭环。

- [x] 9.5.1 **类型 A 处置(plan-merged-with-design,disputed-permanent-drift)**:写 `review/codex_plan_review.md` + `review/plan_cross_check.md` 两份 thin evidence,frontmatter `aligned_with_contract: false` + `drift_decision: disputed-permanent-drift` + `drift_reason ≥ 50` 字解释 plan stage 与 Pre-P0 + P0 design stage 合并的事实 + `reasoning_notes_anchor: reasoning-notes-plan-merged-with-design`;`design.md` `## Reasoning Notes` 段加 §11.5 子段 + `> Anchor: reasoning-notes-plan-merged-with-design` + ≥ 60 非空白字符段落记录用户裁决 + reason + 适用范围(本 change 一次性 self-host bootstrap;未来 change 不适用)。两份 evidence 经 finish_gate 全检通过(disputed-permanent-drift 协议 / cross-check 4 段 / disputed_open=0 / 12-key 8 always-required + 4 conditional)
- [x] 9.5.2 **类型 B 处置(check_tasks_unchecked stage-aware fix-in-tool)**:`tools/forgeue_finish_gate.py::check_tasks_unchecked` 加 `_SELF_STAGE_SECTION_THRESHOLD = 9` 常量 + `_SECTION_HEADING_RE` regex,跟踪 `tasks.md` 当前 `## N` 章节号;`N >= 9` 的 `[ ]` 行不 raise blocker(豁免 §9 P8 finish-gate self-stage / §10 P9 archive / §11 footer),早期 `§1-§8` 仍 raise(workflow-prerequisite stage)+ `(SKIP` / `SKIP:` inline 标记仍跨章节通用;`tests/fixtures/forgeue_workflow/builders.py` 默认 unchecked 章节从 §99 / §98 改为 §4 / §5(都 < 9)以保持原 blocker 测试语义;`design.md` §3 加 "tasks_unchecked stage-aware filter" 段写回(P8 §9.5 fix-in-tool)+ §5 Tool Design 表 `forgeue_finish_gate` 行 "tasks unchecked" 改为 "tasks unchecked(stage-aware,§≥9 self-stage 豁免)" + 2 fence test(`test_finish_gate_skips_p8_p9_self_stage_unchecked` / `test_finish_gate_does_not_skip_pre_p8_unchecked`)
- [x] 9.5.3 **类型 C 处置(openspec subprocess Windows shim resolution,fix-in-tool)**:`tools/forgeue_finish_gate.py` import `shutil` + 新增 `_resolve_openspec_executable()` helper 用 `shutil.which("openspec")` 解析 npm shim;`run_openspec_validate` 改为 resolved-path 优先(原 plain `subprocess.run(["openspec", ...])` 在 Windows 不识别 `.cmd` shim 致 `FileNotFoundError`,即使 user 交互 shell 找得到 `openspec.cmd`);未解析时返回 `openspec_cli_missing` blocker 不再 attempt subprocess;`run_openspec_validate` 现 §9.1 contract 字面 `python tools/forgeue_finish_gate.py --change ... --json`(不带 `--no-validate`)在 Windows + npm `openspec` 装置下 exit 0 + `openspec validate --strict` 通过 + 2 fence test(`test_run_openspec_validate_resolves_via_shutil_which` mock `shutil.which` + `subprocess.run` 验 argv[0] 是 resolved path / `test_run_openspec_validate_returns_blocker_when_unresolved` 验 `shutil.which` None 不 attempt subprocess.run)
- [x] 9.5.4 全量回归 — `python -m pytest -q` 1138 passed in 39.00s(1133 baseline + 5 新增 fence:2 stage-aware + 2 shim resolution + 1 余量;P8 不引入新 evidence_type / 不调 paid provider / 不 mock pytest / stdlib only / 7 ASCII 标记 / `--dry-run` 无副作用全部保持)

P8 evidence(`review/codex_plan_review.md` + `review/plan_cross_check.md`)frontmatter 在本 commit 落 `disputed-permanent-drift`(`writeback_commit: null` 是 disputed 协议本身,不是漏标);`verification/finish_gate_report.md` 由 finish_gate 自动产出 frontmatter `aligned_with_contract: true` + `drift_decision: null`(0 blockers PASS)。沿 P3 `5dd870c` → `1c0da37` / P4 `37288fe` → `2aceee3` / P7 `fe184a6` → `6681e83` 的 resolution-commit 模式;P8 evidence-backfill 仅针对 P8 commit 后 finish_gate_report 自动重新生成时 `writeback_commit` 仍 null(disputed-permanent-drift 协议不需要 writeback_commit,frontmatter 协议层已校验)。
