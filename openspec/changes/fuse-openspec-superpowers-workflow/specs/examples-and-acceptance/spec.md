...
- `tests/unit/test_forgeue_cross_check_format.py` — A/B/C/D section presence + `disputed_open` field validity. (P7 codex F-F written-back-to-spec: the `## A. Claude's Decision Summary (frozen before codex run)` rule is a process protocol enforced by Claude as part of the cross-check workflow — it is NOT a machine-verified timestamp comparison because YAML frontmatter does not carry a frozen-at vs codex-started-at chronology. Tests guard the section structure and `disputed_open` integrity; the freeze ordering is human-attested in the cross-check evidence body, not asserted by the test suite.)
