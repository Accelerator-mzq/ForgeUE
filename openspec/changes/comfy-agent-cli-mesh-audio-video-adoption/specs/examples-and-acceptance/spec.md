...
2. **Mesh step** (e.g. `mesh_step`):
   ...
   - `step.config.spec.comfy_image_param_key: "image_path"` (or whatever key the selected manifest uses for image input — `input_image` / `image` / `source_image`; default `"image_path"` per provider-routing spec D8)
