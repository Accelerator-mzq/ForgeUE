...
- Invoke `unreal.FileMediaSourceFactory()` + `unreal.AssetImportTask` with `filename=<Movies/...>` path + `destination_path=<asset_root>/<run_id>/MS_<base>` (asset_root sourced from `entry["target_package_path"]` parent, sweep-mirror of `domain_audio.import_audio_entry` pattern)
- Set `unreal.FileMediaSource.file_path` editor property to the relative path `Movies/<run_id>/MS_<base>.mp4` (relative to `Content/`, UE runtime resolves this) — this is the FileMediaSource asset's reference to the external mp4 file
- Apply `import_options` from the manifest entry: `loop` / `play_on_open` set as editor properties on the resulting MediaSource asset (or stored as metadata_overrides if FileMediaSource doesn't directly expose those properties — implementer decides per UE 5.x API surface)
...
- **THEN** the source mp4 file is copied to `<project_root>/Content/Movies/<run_id>/MS_<base>.mp4` (NOT `Content/Generated/<run_id>/`); the stub `unreal.FileMediaSourceFactory()` is invoked; `unreal.AssetImportTask` is constructed with `filename=<Movies/...>` and `destination_path=Generated/<run_id>/MS_<base>`; the resulting `FileMediaSource.file_path` property is set to `Movies/<run_id>/MS_<base>.mp4`; the function returns `{"status": "success", "asset_path": "Generated/<run_id>/MS_<base>", ...}`; `tests/integration/test_p4_ue_manifest_only.py::test_p4_domain_video_copies_mp4_to_content_movies_subdir` + `::test_p4_domain_video_creates_file_media_source_uasset_in_content_generated_subdir` fence this
