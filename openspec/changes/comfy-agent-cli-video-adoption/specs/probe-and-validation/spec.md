...
- When opted in, run a single end-to-end video generation via `examples/comfy_local_smoke_video.json`-equivalent params, capture the mp4 / webm bytes, validate magic bytes, and emit `[OK]` / `[FAIL]` / `[SKIP]` ASCII markers (no emoji per the existing "ASCII output markers" Requirement)
