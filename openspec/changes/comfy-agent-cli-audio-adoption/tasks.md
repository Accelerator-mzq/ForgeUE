...
- [x] 1.1 确认前置 change `2026-05-03-comfy-agent-cli-mesh-audio-video-adoption` 已归档,`ComfyAgentWorker` image+mesh-mode 在 head 通过(`python -m pytest tests/unit/test_comfy_subprocess.py -v` 全绿,基线 1234 不退化;若实测有偏差以 `python -m pytest -q` 实数为准,**不**硬编码总数)
- [x] 1.2 在装了 ComfyUI 的机器上跑 `python -m comfyui_api list` 拿真实 manifest 列表;确认 `Audio_Workflows/audio_stable_audio_example` 和 `Audio_Workflows/audio_ace_step_1_t2a_instrumentals` 出现在列表里;记录到 `notes/manifest_audit_<date>.md`
- [x] 1.3 跑 `python -m comfyui_api params --workflow Audio_Workflows/audio_stable_audio_example` 拿 params schema;确认 `text`(REQUIRED)+ `negative_prompt` + `duration_seconds` + `seed` + `steps` + `filename_prefix`;记录到 manifest_audit notes(同上)
- [x] 1.4 起新分支 `feat/openspec-comfy-audio`(从 `main` 拉),或在现有 openspec 分支续加 commit
...
- [x] 1.5b implementation 阶段补全 probe(**non-blocking**,与 §11 L2 evidence 同时跑):用户启 `python -m factory_v3 serve` + Stable Audio Open 模型权重就绪后,跑 `python -m comfyui_api run --workflow Audio_Workflows/audio_stable_audio_example --params '{"text":"test","duration_seconds":5.0,"seed":42,"steps":10}' --project test_audio_probe --lifecycle none --timeout 180` 拿真实 stdout JSON;若与 §1.5 静态阅读结论有偏差(如 `extract_outputs` 实际行为与 source 不同),走 round-2 design / spec / tasks 修订(沿 Phase 1 R5 D10 修订模式)
- [x] 1.6 确认选定 manifest 不依赖远端 API key(Stable Audio Open 模型权重首次拉自 HuggingFace 后纯本地);若 ACE-Step custom node 未装,跳过 1.5 的 ACE-Step 探活并在 manifest_audit notes 记录(`notes/manifest_audit_<date>.md`)
- [x] 1.7 跑 codex S2 design adversarial review(`/codex:adversarial-review` 对 design.md;沿 Phase 1 round 1-2 模式),拿 codex 输出 verbatim 落 `review/codex_design_review_round1.md` + 12-key audit frontmatter;若 codex raise high/medium finding,先 design.md writeback round 2 再继续 §2(沿 Phase 1 D3 round-by-round 节奏);若 codex 全 low / no finding,直接进 §2

## 2. AudioWorker baseline 新建(commit 1)

> **依赖**:无(纯新建)。`mesh_worker.py` 作为参考模板,本 task 复制 + 命名替换。

- [x] 2.1 新建 `src/framework/providers/workers/audio_worker.py`,顶部 `from __future__ import annotations` + 标准 imports(typing / dataclasses / abc)
- [x] 2.2 加 `AudioCandidate` dataclass(类比 `MeshCandidate`):
  ```python
  ...
  ```
- [x] 2.3 加异常树(类比 `MeshWorkerError` 三层):
  ```python
  ...
  ```
- [x] 2.4 加 `AudioWorker(ABC)` ABC(类比 `MeshWorker`):
  ```python
  ...
  ```
- [x] 2.5 加 `FakeAudioWorker(AudioWorker)` 测试 fixture:返回 minimal valid FLAC bytes(magic `b"fLaC"` + STREAMINFO block + minimal frame header,~50 bytes),不依赖第三方 codec。`num_candidates` 个相同 candidates(metadata 加 `is_fake: True` 标识)
- [x] 2.6 `tests/unit/test_audio_worker.py` 新建,加 5 fence:
...
- [x] 2.7 commit 1:`feat(audio): introduce AudioWorker ABC + AudioCandidate dataclass + exception tree (TBD-002 lift)`

## 3. ModelRegistry config 扩展(commit 2)

- [x] 3.1 在 `config/models.yaml` `models:` 段加 `comfy_local_audio` entry:
  ```yaml
  comfy_local_audio:
...
- [x] 3.2 在 `config/models.yaml` `aliases:` 段加 `audio_local` alias:`preferred: ["comfy_local_audio"]` + `fallback: []`(无远端 audio worker fallback;留 follow-on `audio-worker-audiocraft-adoption`)
- [x] 3.3 `providers.comfy_api` entry **不动**(image / mesh change 已加,沿用)
- [x] 3.4 `tests/fixtures/test_models.yaml` 同步加 `comfy_local_audio` + `audio_local`(用于 unit test 不污染 production yaml)
- [x] 3.5 `tests/unit/test_model_registry.py` 加 2 fence:
  - `test_comfy_local_audio_model_resolves_via_audio_local_alias`
  - `test_audio_local_alias_kind_is_audio`
- [x] 3.6 commit 2:`feat(registry): add comfy/local-audio virtual model + audio_local alias`

## 4. ComfyAgentWorker capability-aware 扩 audio(commit 3)

- [x] 4.1 在 `src/framework/providers/workers/comfy_worker.py` 扩 4 个类常量字典 audio entry:
  ```python
  ...
  ```
- [x] 4.2 在 `ComfyAgentWorker` 加新方法 `generate_audio(spec, num_candidates, seed, timeout_s) -> list[AudioCandidate]`(NOT part of `ComfyWorker` ABC;F4+F5 round-1 design + F-Plan-3+F-Plan-4 round-2 plan 修订:扩展名 + **强制** magic bytes 二次校验 + **per-candidate loop** + **path trust-boundary 防护**(`is_file` + `is_symlink`);duration / sample_rate 顶层 None):
...
- [x] 4.3 `__init__` 守门错误消息列表自动包含 `comfy/local-audio`(因 4.1 字典扩展;无需手动改 error string,但检查 message 现在列出 3 个 supported ids)
- [x] 4.4 `tests/unit/test_comfy_subprocess.py` 加 audio fence(具体名见 specs/probe-and-validation/spec.md "ComfyUI audio capability dispatch has dedicated regression fences" Requirement 列表):
...
- [x] 4.5 commit 3:`feat(comfy): extend ComfyAgentWorker with audio capability dispatch + generate_audio method`
...
- [x] 5.1 新建 `src/framework/runtime/executors/generate_audio.py`,框架参考 `generate_image.py`(text-to-something 模式;**NOT** `generate_mesh.py` 的 image-to-something 模式因没 source bytes)
- [x] 5.2 实现 `GenerateAudioExecutor` 类(F1 round-2 修订:沿用现有 `Step.type=StepType.generate` + `capability_ref` 路由,不新增 step type;F2 round-2 修订:retry/wrap 三 except 块拆分,沿用 mesh `generate_mesh.py:160-172` 模式):
...
- [x] 5.3 在 `src/framework/runtime/executors/__init__.py` 加 import,**不**自动注册到 registry(沿 image / mesh 模式 — registry 注册在 framework.run)
- [x] 5.4 在 `src/framework/run.py` `ExecutorRegistry` setup 段加 `registry.register(GenerateAudioExecutor(...))`(对照 generate_image / generate_mesh registration 写法);**不**改 `loader.py`(loader 仅做 `Step.model_validate`,无 step-kind 表;F1 round-2 修订)
- [x] 5.5 `tests/unit/test_generate_audio_comfy.py` 新建,加 fence(F2 round-2 修订:fence 名按 mesh 风格 + 加 deterministic short-circuit fence):
...
- [x] 5.6 `tests/unit/test_workflow_loader.py` 加 2 fence(F1 round-2 修订:fence 名 step_kind → capability_ref):`test_audio_t2a_capability_ref_dispatches_to_generate_audio_executor` + `test_audio_t2a_capability_ref_rejects_hardcoded_model_id_without_alias`
- [x] 5.7 commit 4:`feat(executor): introduce GenerateAudioExecutor + audio.t2a capability_ref registration in ExecutorRegistry`(F-Plan-R4-C round-4 修订:commit title 不写 "step type registration",真实是 `(StepType.generate, "audio.t2a")` entry 在 `framework.run` 注册)

## 6. FailureModeMap audio_worker_* mode(commit 5)

- [x] 6.1 在 `src/framework/runtime/failure_mode_map.py` 加 2 个 audio mode entry:
...
- [x] 6.2 在 `FailureModeMap.from_exception` 加分类(顺序至关重要,wrapped audio 异常必须**在** generic ComfyWorker / WorkerTimeout 之前匹配):
  ```python
  ...
  ```
- [x] 6.3 `tests/unit/test_failure_mode_map.py` 加 fence(沿 Phase 1 R4-F1 sweep 模式覆盖所有 inner exc → wrapped → mode → Decision 链路):
...
- [x] 6.4 commit 5:`feat(failure-mode): map AudioWorkerTimeout / AudioWorkerUnsupportedResponse to abort_or_fallback`

## 7. DryRunPass extension(commit 6)

- [x] 7.1 在 `src/framework/runtime/dry_run_pass.py` `_check_comfy_reachability` 方法的 gate set 从 `{"comfy/local", "comfy/local-mesh"}` 扩为 `{"comfy/local", "comfy/local-mesh", "comfy/local-audio"}`(沿 Phase 1 P-F4 round-2 plan writeback 模式)
- [x] 7.2 探活逻辑不变(`ComfyAgentWorker.probe_sync(scripts_dir=...)` 跑一次 `python -m comfyui_api status` timeout 30s)
- [x] 7.3 `tests/unit/test_dry_run_pass.py` 加 1 fence:`test_dry_run_probes_comfy_when_comfy_local_audio_in_routes`(沿 mesh 模式)
- [x] 7.4 commit 6:`feat(dry-run): extend ComfyUI reachability probe gate to include comfy/local-audio`

## 8. examples/comfy_local_smoke_audio.json bundle(commit 7)

- [x] 8.1 新建 `examples/comfy_local_smoke_audio.json`(F-Plan-1 + F-Plan-6 round-2 plan-stage 修订:bundle 真实顶层三段 `task` / `workflow`(无嵌 steps)/ `steps` 并列,对照 [examples/comfy_local_smoke.json](examples/comfy_local_smoke.json) + [examples/comfy_local_smoke_mesh.json](examples/comfy_local_smoke_mesh.json) 实测 schema;`worker_timeout_s` 在 `step.config` 内,**不**在 `retry_policy`(`RetryPolicy` schema 仅含 `max_attempts/backoff/retry_on`,无 `timeout_seconds`)):
  ```json
  {
    "task": {
...
- [x] 8.2 `tests/integration/test_example_bundles_smoke.py` 加 1 fence:`test_comfy_local_smoke_audio_loads_with_audio_local_alias_and_no_workflow_graph`(沿 image / mesh 模式 — 仅 loader-level invariants,不跑 worker)
- [x] 8.3 commit 7:`feat(examples): add comfy_local_smoke_audio.json bundle (text-to-audio single step)`

## 9. probes/provider/probe_comfy_audio.py(commit 8)

- [x] 9.1 新建 `probes/provider/probe_comfy_audio.py`,沿 Phase 1 `probe_comfy_mesh.py` 模板(若不存在 mesh probe,沿 `probes/provider/probe_*` 任一 file 风格)
- [x] 9.2 实装:
...
- [x] 9.3 `tests/unit/test_probe_framework.py` 加 1 fence:`test_probe_comfy_audio_default_skip_without_optin`(沿 mesh / image probe 模式)
- [x] 9.4 commit 8:`feat(probes): add probe_comfy_audio.py opt-in audio smoke (FORGEUE_PROBE_COMFY_AUDIO=1)`

## 10. Documentation Sync Gate(commit 9-12,沿 Phase 1 split-by-doc 模式)

- [x] 10.1 commit 9 `docs(srs+lld): document audio worker baseline + ComfyUI audio capability`:
...
- [x] 10.2 commit 10 `docs(hld+test_spec): document audio capability dispatch + fence indices`:
...
- [x] 10.3 commit 11 `docs(acceptance+changelog): document Phase 2 audio status`:
...
- [x] 10.4 commit 12 `docs(claude+agents): update ComfyUI audio smoke + audio worker section`:
...
## 11. L2 evidence — 本机跑 audio live smoke(commit 13)— **FULL PASS**

**Status (2026-05-03):L2 evidence FULL PASS**。第一次跑被 ComfyUI 0.9.2 user-
authored workflow JSON 上游 bug 阻断(`SaveAudioMP3` 缺 `quality` required input,
HTTP 400);用户授权 Claude 改 ComfyUI workflow JSON 一行(`SaveAudioMP3` →
`SaveAudio`,匹配 manifest `outputs.primary: audio/flac` 原本声明),重跑拿真实
FLAC 1.17 MB(magic bytes `fLaC` + STREAMINFO 头 PASS)。

L2 PASS evidence:`notes/live_smoke_audio_20260503_full.md`(supersedes earlier
deferred note `notes/live_smoke_audio_blocked_20260503.md`)。

- [x] 11.1 用户准备:Stable Audio Open 1.0 模型权重已缓存(无 HuggingFace 拉延迟)
- [x] 11.2 终端 1:`python -m factory_v3 serve` 启 ComfyUI(task `bygdeokhf`,17.7s 冷启动)
- [x] 11.3 终端 2 跑 `framework.run` smoke — **完成 (run_id=audio_smoke_l2_pass; status=succeeded)**:
  ```bash
  PYTHONPATH=src \
  FORGEUE_COMFY_SCRIPTS_DIR=D:/AI/ComfyUI/scripts \
  python -m framework.run --task examples/comfy_local_smoke_audio.json --live-llm \
    --run-id audio_smoke_l2_pass --artifact-root artifacts/2026-05-03
  ```
- [x] 11.4 验证 L2 evidence 客观判定 — **PASS**:
  - (a) `artifacts/2026-05-03/audio_smoke_l2_pass/audio_smoke_l2_pass_step_audio_cand_audio_0.flac` 存在
  - (b) 文件大小 1,227,925 bytes(1199.1 KB)远超 100 KB 阈值
  - (c) magic bytes 前 4 字节 `b'fLaC'` + STREAMINFO header(`0x00 0x00 0x00 0x22`)PASS
  - (d) duration 校验留 follow-on `audio-metadata-parser` change(本 change scope 不变)
- [x] 11.5 evidence 文件 `notes/live_smoke_audio_20260503_full.md` 记录:命令行 / run_id / artifact_id / 文件大小 / magic bytes / producer attribution(`comfy_agent_cli` / `comfy/local-audio` 正确)/ metadata 三键 single-source 验证
- [x] 11.6 commit 13:`docs(openspec): G6+G11 codex review verbatim + cross-checks + L2 actual PASS evidence`(212208f)— L2 evidence note + G6/G11 cross-check + design.md G11-F2 反驳 + follow-on change scaffold
