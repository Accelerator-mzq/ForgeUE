...
- [x] 1.1 写 `notes/pre_p0/subagent_dogfood_protocol.md`(沿 `fuse-openspec-superpowers-workflow/notes/pre_p0/forgeue-fusion-claude.md` 范式):说明 Claude 主 session 如何用 Task tool 手工模拟 implementer / spec reviewer / code quality reviewer subagent dispatch;包含 4 段:dispatching protocol / per-task evidence schema / final review trigger / chicken-and-egg 处理
- [x] 1.2 写 `notes/pre_p0/plan_cross_check.md`(plan-level cross-check,沿 design.md §3 Cross-check Protocol 4 段结构 ## A/B/C/D):`## A` 冻结于 codex hook 调用之前;`## B` 在调 `/codex:adversarial-review --background "<本 change 整体方案>"` 之后填;disputed_open=0 才能进 §2
- [x] 1.3 codex 调 `/codex:adversarial-review --background "本 change 整体方案 + design.md 8 项 D 决策 + tasks.md self-host bootstrap 模式"`,产物落 `notes/pre_p0/codex_review_round1.md`
- [x] 1.4 Claude 独立验证 codex 每条 finding(沿 ForgeUE memory `feedback_verify_external_reviews`);填 `plan_cross_check.md` ## B/C/D;blocker 涉及 design 必回写 design.md 或标 disputed-permanent-drift
- [x] 1.5 `disputed_open == 0` + `notes/pre_p0/` 全部 frontmatter 完整 + `tools/forgeue_change_state.py --change adopt-subagent-driven-development --writeback-check --json` exit 0 → 进 §2

## 2. 文档同步(ForgeUE workflow 文档层)

- [x] 2.1 编辑 `docs/ai_workflow/forgeue_integrated_ai_workflow.md` §B.3 表:
...
- [x] 2.1b 编辑 `docs/ai_workflow/forgeue_integrated_ai_workflow.md` §B.1 状态机表(line 113-124):
  - **S3 行** "允许命令" 列 `ForgeUE change-{apply,debug,status}` → `ForgeUE change-{apply-subagent,apply-direct,debug,status}`
  - **S4 行** 同 S3 行同款修改
- [x] 2.2 编辑 `docs/ai_workflow/forgeue_integrated_ai_workflow.md` §B.4 表后加新段 §B.6 `subagent-driven-development 集成边界`,描述 4 类 per-task evidence_type + 命令分流(`change-apply-subagent` vs `change-apply-direct`)
- [x] 2.3 编辑 `docs/ai_workflow/forgeue_integrated_ai_workflow.md` §A.6 命令边界段:加 `change-apply-subagent` / `change-apply-direct` 两个 ForgeUE 命令的描述行;现 `change-apply` 行标 deprecated
- [x] 2.4 编辑 `docs/ai_workflow/README.md` §5 Agent 分工表 Superpowers 行:删 `subagent-driven-development paid API 拦截(env guard + ADR-007)` 描述,改成 `subagent-driven-development default for /forgeue:change-apply-subagent;ADR-009 token-budget tracker informational`;`using-git-worktrees 禁用` 改 `using-git-worktrees REQUIRED for subagent path`
- [x] 2.4b 编辑 `docs/ai_workflow/README.md` §8 入口表(line 283):"进入 S3→S4-S5:implementation" 行 `/forgeue:change-apply <id>(...)` → 拆 2 行:`/forgeue:change-apply-subagent <id>(default;invoke superpowers:subagent-driven-development + 4 类 per-task evidence + budget tracker informational)` + `/forgeue:change-apply-direct <id>(fallback;executing-plans + TDD + tdd_log/debug_log)`
- [x] 2.4c 编辑 **repo 根 `README.md`**(非 `docs/ai_workflow/README.md`)line 380-388 ForgeUE 命令清单表:`/forgeue:change-apply` 行(line 383)拆成 `/forgeue:change-apply-subagent`(default subagent dispatch + 4 类 per-task evidence + budget tracker informational)+ `/forgeue:change-apply-direct`(fallback executing-plans + TDD)两行;命令总数从 8 → 9
- [x] 2.5 编辑 `CLAUDE.md` `### ForgeUE Integrated AI Change Workflow` 段:更新 8 个 `/forgeue:change-*` 命令描述,把 `change-apply` 拆成 `change-apply-subagent` + `change-apply-direct`(命令数从 8 → 9);加 ADR-009 引用
- [x] 2.6 编辑 `AGENTS.md` 同段(与 `CLAUDE.md` 对齐;Codex CLI / Cursor / Aider 视角)
- [x] 2.7 编辑 `.claude/skills/forgeue-integrated-change-workflow/SKILL.md` Superpowers 集成边界表 + 命令清单;沿 `forgeue_integrated_ai_workflow.md` §B.3 / §B.6 同步
- [x] 2.8 编辑 `docs/ai_workflow/forgeue_quickstart.md`(3 处必改):
...
- [x] 2.9 验证 `docs/ai_workflow/validation_matrix.md` 不需要改(grep 已确认无 `change-apply` 字面引用,只涉及 Level 0/1/2 验证矩阵,与命令拆分无关)

## 3. SRS / acceptance_report 加 ADR-009

- [x] 3.1 编辑 `docs/requirements/SRS.md` § ADR 表:在 ADR-007 行后新增 ADR-009 行
...
- [x] 3.2 编辑 `docs/acceptance/acceptance_report.md` § ADR 状态表:新增 ADR-009 行,状态 `✅ 已批准 + 工具实施待 §6 完成`

## 4. ForgeUE 命令重构(`/forgeue:change-apply` 拆 2)

- [x] 4.1 新建 `.claude/commands/forgeue/change-apply-subagent.md`(default subagent dispatch 路径):
...
- [x] 4.2 新建 `.claude/commands/forgeue/change-apply-direct.md`(executing-plans fallback 路径):
...
- [x] 4.3 改写现 `.claude/commands/forgeue/change-apply.md`:删除现 step 流程内容,替换为 deprecated banner:
  ```markdown
  ...
  ```
- [x] 4.4 验证现 `.claude/skills/forgeue-integrated-change-workflow/SKILL.md` 引用更新:把 `/forgeue:change-apply` 替换为 `/forgeue:change-apply-{subagent,direct}` 二选一

## 5. `forgeue_finish_gate.py` 扩 evidence_type enum

- [x] 5.1 编辑 `tools/forgeue_finish_gate.py` `_VALID_EVIDENCE_TYPES` 等 enum 集合,加 4 项:
  - `subagent_implementer_report`
  - `subagent_spec_review`
  - `subagent_code_quality_review`
  - `subagent_final_review`
- [x] 5.2 编辑同文件 `_DEFAULT_PATHS_BY_TYPE`(默认 path 表),加 4 项:
...
- [x] 5.3 完整性检查规则(F2 修复 — 不依赖 helper marker file):`change-apply-subagent` 路径下 4 类 evidence_type **全部 REQUIRED**;`change-apply-direct` 路径下 4 类 evidence_type 都不 REQUIRED(沿用 `tdd_log` / `debug_log` 即可)。**判定路径改为从 evidence frontmatter `triggered_by_command` 字段扫描**:
...
- [x] 5.4 fence test `tests/unit/test_forgeue_finish_gate.py` 加 case(F2 + F1 修复):
...
- [x] 5.5 编辑 `tools/forgeue_change_state.py` DRIFT detector(F3 修复;line 369 + line 396):
...
- [x] 5.6 加 fence test `tests/unit/test_forgeue_change_state.py`(F3 fence):
...
- [x] 5.7 修 task 2 引入的 fence count regression(task 3 §5 dogfood code_quality_reviewer 独立验证暴露;`evidence_exposes_contract_gap` DRIFT writeback,16 errors outstanding):
...
- [x] 6.1 新建 `tools/forgeue_subagent_budget.py`(stdlib only,~100 行,沿 forgeue_finish_gate.py 范式):
...
- [x] 6.2 新建 `tests/unit/test_forgeue_subagent_budget.py` fence test:
...
- [x] 6.3 跑 `python -m pytest tests/unit/test_forgeue_subagent_budget.py -v` 全绿

## 7. capability spec delta sync(自动,archive 时跑)

- [x] 7.1 验证 `openspec/changes/adopt-subagent-driven-development/specs/examples-and-acceptance/spec.md` 通过 strict validate(本任务在 §10.1 Finish Gate 中跑,本条只确认 spec delta 文件存在);archive 时 `openspec archive` 自动跑 sync-specs 把 ADDED Requirement 合入 `openspec/specs/examples-and-acceptance/spec.md` 主 spec

## 8. Level 0 验证

- [x] 8.1 跑 `python -m pytest -q`(预期 549 + 新增 ~5 fence test = ~554 全绿;以实测为准,**不硬编码总数**)
- [x] 8.2 跑 `python tools/forgeue_subagent_budget.py --status --change adopt-subagent-driven-development`(可执行,JSON 格式正确)
- [x] 8.3 跑 `python tools/forgeue_finish_gate.py --change adopt-subagent-driven-development --json`(预期不 FAIL;evidence 完整性从 evidence frontmatter `triggered_by_command` 字段判定 dispatch mode,沿 §5.3 修正后规则,**不依赖** `dispatch_mode.txt` helper marker)
- [x] 8.4 跑 `python tools/forgeue_change_state.py --change adopt-subagent-driven-development --writeback-check --json`(预期 exit 0)
- [x] 8.5 写 `verification/verify_report.md`(`evidence_type: verify_report`,12-key frontmatter):body summary `[OK]: N / [FAIL]: 0 / [SKIP]: M`;Level 1/2 SKIP 必有 reason(本 change 不涉及 LLM live / paid provider / UE,Level 1/2 全 SKIP)

## 9. S6 review

- [x] 9.1 invoke `superpowers:requesting-code-review` skill 跑 self-review;产物落 `review/superpowers_review.md`(`evidence_type: superpowers_review`)
- [x] 9.2 codex 调 `/codex:adversarial-review --background "<本 change 整体 + 实施 + evidence>"` mixed scope post-implementation review;产物落 `review/codex_adversarial_review.md`
- [x] 9.3 Claude 独立验证 codex 每条 finding(沿 ForgeUE memory `feedback_verify_external_reviews`);blocker 涉及 design 必回写或标 disputed-permanent-drift
- [x] 9.4 review blocker 0 + superpowers_review.md frontmatter `aligned_with_contract: true`(或带 drift 标记)→ 进 §10
