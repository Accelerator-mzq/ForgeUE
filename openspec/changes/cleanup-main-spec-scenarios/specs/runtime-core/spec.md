...
The system SHALL, when `BudgetTracker.assert_within(...)` raises `BudgetExceeded` (`src/framework/runtime/budget_tracker.py:38-83`) on a cost cap miss, have `Orchestrator` (`src/framework/runtime/orchestrator.py`) catch the exception and terminate the Run by:

- setting `run.metrics["termination_reason"]` to `"budget_exceeded(cap=<cap>, spent=<spent>)"`,
- setting `run.metrics["last_failure_mode"] = "budget_exceeded"`,
- appending a `failure_event` whose `decision` tag is `"human_review_required"` (a real `Decision` enum member declared in `framework.core.enums.Decision`),
- setting `run.status = RunStatus.failed`, and
- returning a terminating `_StepOutcome(terminate=True, next_step_id=None)` so no further Step executes.

The Run MUST NOT silently exit. The cap-miss path is exercised both at fresh-execution time (`orchestrator.py:566-580` after `budget_tracker.record(...)` for the just-completed Step) and at fresh-process resume time (`orchestrator.py:428-435` when a cached Step's persisted `cost_usd` is replayed into a freshly-constructed `BudgetTracker`); both paths produce identical termination metrics so resume behaves like fresh-execution under cap miss. The Requirement title `Budget exceeded synthesizes a Verdict` is preserved as a historical name; the authoritative termination mechanism is the `BudgetExceeded → Orchestrator` direct path described above, NOT a synthesised `Verdict` flowing through `TransitionEngine` (the historical phrasing referenced a `budget_exceeded` enum value that does not exist in `framework.core.enums.Decision`).

#### Scenario: BudgetTracker.assert_within raises BudgetExceeded; Orchestrator records termination_reason and fails the Run without silent exit

- GIVEN a Run whose `task.budget_policy.total_cost_cap_usd` is set, with a Step whose execution (or, on resume, a cached Step's replayed `cost_usd`) pushes `BudgetTracker.spend.total_usd` above the cap
- WHEN `BudgetTracker.check()` returns False (cap miss) — equivalently, `BudgetTracker.assert_within(...)` raises `BudgetExceeded(RuntimeError)` per `src/framework/runtime/budget_tracker.py:81-83` — and `Orchestrator` reaches the cap-miss branch (line 566-580 fresh-execution, or line 428-435 resume cache-hit cost replay)
- THEN the `Orchestrator` records `run.metrics["termination_reason"]` to `"budget_exceeded(cap=<cap>, spent=<spent>)"`, sets `run.metrics["last_failure_mode"] = "budget_exceeded"`, appends `result.failure_events` with `decision: "human_review_required"`, sets `run.status = RunStatus.failed`, and returns `_StepOutcome(terminate=True, next_step_id=None)` so subsequent Steps are skipped; `tests/unit/test_budget_tracker.py::test_assert_within_raises_when_over_cap` (line 49) fences the BudgetTracker side, and the Run-level invariants are exercised end-to-end via `tests/unit/test_review_budget.py` and the cache-hit replay path described in `Cost is persisted before Checkpoint`'s existing Scenario; the Run MUST NOT silently exit, and the termination metrics MUST be identical between fresh-execution and resume cache-hit cap miss
