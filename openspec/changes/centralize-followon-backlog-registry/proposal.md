...
- **新增** 一次性 backfill 22 active follow-on 项(沿 user 拍板决策点 3(a);**adapted for fix-finish-gate-archived-replay-compat merge `88a8aec`** — 原 9 workflow-protocol 中 2 项已 closed,直接进 archived.md tombstone 不进 active):7 archive tracking + 9 SRS TBD pointer + 6 多模态 capability boundary。详见本 change `tasks.md` P0 backfill 段。
- **修改** `.claude/commands/forgeue/change-finish.md`:`Preflight` 段加 followon continuity check 步骤,调 `forgeue_finish_gate.py --check-followon-continuity`(blocker)。
...
- **22 active follow-on 项**,见 `tasks.md` P0 backfill 段。详细分类:
  - **Workflow protocol**(7 项,缩自原 9 项 — 2 项 closed by `88a8aec` 直接进 archived.md):`fix-video-export-path-split-d12-violation` / `fix-run-import-skipped-filter-permission-only` / `enhance-workflow-automation-handoff-persistence` / `add-forgeue-brainstorm-stage` / `enhance-workflow-automation-finishing-branch` / `enhance-workflow-automation-final-review-fence-strictness` / `analyze-superpowers-skills-openspec-integration-gaps`
