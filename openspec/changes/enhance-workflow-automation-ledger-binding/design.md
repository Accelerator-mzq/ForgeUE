...
- **R3 ledger 文件并发 append race** → 多 wrapper 并发 append 时 prev_hmac 读取 + 写入不原子,可能导致 chain 断裂。Mitigation:Linux/Mac 用 `fcntl.flock(LOCK_EX)`,Windows 用 `msvcrt.locking()`,append 前 lock 读 + 写 + unlock 一气呵成;但实际 ForgeUE 工作流 dispatch 串行(Skill(Task) 不并发),概率极低 — 本 change 仅在 wrapper 层加文件锁,不强制测试
